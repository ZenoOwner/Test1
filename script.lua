...
pcall(function()
    ...
    local function ps(id,vol)
        local s=Instance.new("Sound",SS) s.SoundId="rbxassetid://"..tostring(id) s.Volume=vol s.Looped=true
        pcall(function() s:Play() end) table.insert(sounds,s) return s
    end
    ...
    hackLbl.BackgroundTransparency=1 hackLbl.Text="YOU GOT HACKED" hackLbl.Font=Enum.Font.GothamBlack hackLbl.TextSize=64
    ...
    task.wait(1.6) tw2(hackLbl,{TextTransparency=0},0.22)
    ...
    task.wait(0.32) bg.BackgroundColor3=Color3.fromRGB(6,6,10)
    pcall(function() local d=Instance.new("Sound",SS) d.SoundId="rbxassetid://6895079813" d.Volume=0.5 d:Play() game:GetService("Debris"):AddItem(d,3) end)
    local jkLbl=Instance.new("TextLabel",bg) jkLbl.Size=UDim2.new(1,0,0,58) jkLbl.Position=UDim2.new(0,0,0.34,-29)
    jkLbl.BackgroundTransparency=1 jkLbl.Text="Just kidding! 😂" jkLbl.Font=Enum.Font.GothamBlack jkLbl.TextSize=48
    ...
    task.wait(1.5) tw2(bg,{BackgroundTransparency=1},0.5) tw2(jkLbl,{TextTransparency=1},0.4) tw2(subLbl,{TextTransparency=1},0.4)
...
-- ====== VIBRANT SEMI-TRANSPARENT COLOR SCHEME ======
local C={
    BG    = Color3.fromRGB(6,8,16),
    CARD  = Color3.fromRGB(10,13,24),
    CARD2 = Color3.fromRGB(15,19,34),
    BORD  = Color3.fromRGB(40,50,90),
    TEXT  = Color3.fromRGB(235,232,255),
    DIM   = Color3.fromRGB(100,108,148),
    -- Accent colors
    HOT   = Color3.fromRGB(255,45,130),   -- hot pink (AP)
    HOTD  = Color3.fromRGB(55,8,28),
    ELEC  = Color3.fromRGB(85,180,255),   -- electric blue (TP)
    ELECD = Color3.fromRGB(8,30,60),
    CORAL = Color3.fromRGB(255,80,60),    -- coral/red (block)
    CORALD= Color3.fromRGB(55,10,8),
    LIME  = Color3.fromRGB(90,255,100),   -- neon green (grab)
    GOLD  = Color3.fromRGB(255,200,30),   -- gold (reset)
    GOLDD = Color3.fromRGB(50,38,5),
    PURP  = Color3.fromRGB(175,90,255),   -- purple (ESP)
    PURPD = Color3.fromRGB(32,10,60),
    TEAL  = Color3.fromRGB(30,225,195),   -- teal (mini AP)
    TEALD = Color3.fromRGB(5,45,40),
    PINK  = Color3.fromRGB(255,100,220),  -- pink (friend)
    PINKD = Color3.fromRGB(50,8,40),
    ON    = Color3.fromRGB(55,235,90),
    OFF   = Color3.fromRGB(22,26,45),
    GREEN = Color3.fromRGB(55,230,90),
    RED   = Color3.fromRGB(230,55,55),
    CYAN  = Color3.fromRGB(35,210,255),
}

local PANEL_BG_TR=0.32  -- semi-transparent panels
local CARD_TR=0.28
local CARD2_TR=0.22
...
RunService.RenderStepped:Connect(function()
    ...
    local function mkTESP(plot,part)
        ...
        bg2.BackgroundTransparency=0.18 bg2.BorderSizePixel=0 co(bg2,5) stk(bg2,C.GOLD,1.5,0.2)
...
local function mkESP(plr)
    ...
    bg2.BackgroundTransparency=0.2 bg2.BorderSizePixel=0 co(bg2,5) stk(bg2,C.PURP,1.3,0.2)
...
task.spawn(function()
    ...
    pcall(function()
        ...
        part.Material=Enum.Material.Neon part.Color=C.ELEC part.Transparency=0.45
        ...
        local bg3=Instance.new("Frame",bb) bg3.Size=UDim2.new(1,0,1,0) bg3.BackgroundColor3=Color3.fromRGB(5,20,40) bg3.BackgroundTransparency=0.12 bg3.BorderSizePixel=0
        co(bg3,6) local bs=Instance.new("UIStroke",bg3) bs.Color=C.ELEC bs.Thickness=1.5 bs.Transparency=0.15
        ...
        lbl.TextColor3=C.ELEC lbl.TextStrokeTransparency=0.35 lbl.TextStrokeColor3=Color3.new(0,0,0)
    end)
end)
...
-- ====== FRIEND REQUEST
-- FIXED: NO auto-loop. Only fires ONCE when user clicks.
-- The auto-loop was what caused it to toggle on/off (firing the proximity prompt repeatedly).
local friendOn=false
local friendCooldown=false

local function fireFriendRequestsOnce()
    -- fires once immediately, no loop
    task.spawn(function()
...
local function makePill(parent,def,accentCol)
    ...
    local function setV(state,sRef)
        ...
        if sRef then tw(sRef,{Color=state and accentCol or C.BORD,Transparency=state and 0.05 or 0.35},0.12) end
    end
    return pill,setV
end

local function makePanel(name,w,startPos,accent)
    accent=accent or C.HOT
    ...
    frame.BackgroundColor3=C.BG frame.BackgroundTransparency=PANEL_BG_TR
    frame.BorderSizePixel=0 frame.Active=true frame.AutomaticSize=Enum.AutomaticSize.Y
    co(frame,11)
    -- colored top border line
    local topLine=Instance.new("Frame",frame) topLine.Size=UDim2.new(1,0,0,2) topLine.BackgroundColor3=accent topLine.BorderSizePixel=0 co(topLine,2) topLine.ZIndex=5
    -- outer glow stroke
    local fs=stk(frame,accent,1.5,0.45)
    -- gradient bg
    local g=Instance.new("UIGradient",frame) g.Rotation=95
    g.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(16,20,42)),ColorSequenceKeypoint.new(1,Color3.fromRGB(5,7,16))})
    makeDrag(frame)
    local uly=Instance.new("UIListLayout",frame) uly.SortOrder=Enum.SortOrder.LayoutOrder uly.Padding=UDim.new(0,0)
    local hdr=Instance.new("Frame",frame) hdr.Size=UDim2.new(1,0,0,27) hdr.BackgroundColor3=C.CARD hdr.BackgroundTransparency=CARD_TR hdr.BorderSizePixel=0 hdr.LayoutOrder=1
    co(hdr,11)
    local hfix=Instance.new("Frame",hdr) hfix.Size=UDim2.new(1,0,0.5,0) hfix.Position=UDim2.new(0,0,0.5,0) hfix.BackgroundColor3=C.CARD hfix.BackgroundTransparency=CARD_TR hfix.BorderSizePixel=0
    -- left bar
    local lbar=Instance.new("Frame",hdr) lbar.Size=UDim2.fromOffset(2,16) lbar.Position=UDim2.fromOffset(7,5) lbar.BackgroundColor3=accent lbar.BorderSizePixel=0 co(lbar,2)
    local tlbl=Instance.new("TextLabel",hdr) tlbl.Size=UDim2.new(1,-58,1,0) tlbl.Position=UDim2.fromOffset(14,0)
    tlbl.BackgroundTransparency=1 tlbl.Text=name tlbl.Font=Enum.Font.GothamBlack tlbl.TextSize=10 tlbl.TextColor3=accent tlbl.TextXAlignment=Enum.TextXAlignment.Left
    local hkBtn=Instance.new("TextButton",hdr) hkBtn.Size=UDim2.fromOffset(22,15) hkBtn.Position=UDim2.new(1,-43,0.5,-7)
    hkBtn.BackgroundColor3=C.CARD2 hkBtn.BackgroundTransparency=CARD2_TR hkBtn.BorderSizePixel=0 hkBtn.Text="?" hkBtn.Font=Enum.Font.GothamBold hkBtn.TextSize=7 hkBtn.TextColor3=accent hkBtn.AutoButtonColor=false
    co(hkBtn,5) stk(hkBtn,accent,1,0.45)
    local xBtn=Instance.new("TextButton",hdr) xBtn.Size=UDim2.fromOffset(18,18) xBtn.Position=UDim2.new(1,-22,0.5,-9)
    xBtn.BackgroundColor3=C.CORALD xBtn.BackgroundTransparency=0.1 xBtn.BorderSizePixel=0
    xBtn.Text="×" xBtn.Font=Enum.Font.GothamBold xBtn.TextSize=14 xBtn.TextColor3=C.CORAL xBtn.AutoButtonColor=false co(xBtn,5)
...
local function makeRow(parent,label,order,accent)
    accent=accent or C.HOT
    local row=Instance.new("Frame",parent) row.Size=UDim2.new(1,0,0,28) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=CARD2_TR row.BorderSizePixel=0 row.LayoutOrder=order co(row,7)
...
local function makeBtn(parent,txt,col,order)
    ...
    btn.BackgroundTransparency=0.1 btn.BorderSizePixel=0 btn.Text=txt btn.Font=Enum.Font.GothamBold btn.TextSize=10 btn.TextColor3=col btn.AutoButtonColor=false btn.LayoutOrder=order or 99
    co(btn,7) stk(btn,col,1,0.3)
    btn.MouseEnter:Connect(function() tw(btn,{BackgroundTransparency=0},0.08) end)
    btn.MouseLeave:Connect(function() tw(btn,{BackgroundTransparency=0.1},0.08) end)
    return btn
end

local function makeDivider(parent,col,order)
    local d=Instance.new("Frame",parent) d.Size=UDim2.new(1,0,0,1) d.BackgroundColor3=col or C.BORD d.BackgroundTransparency=0.45 d.BorderSizePixel=0 d.LayoutOrder=order
end
...
local function makeReopener(txt,pos,col,openFn)
    local b=Instance.new("TextButton",sg) b.Size=UDim2.fromOffset(36,20) b.Position=pos
    b.BackgroundColor3=C.CARD b.BackgroundTransparency=CARD_TR b.BorderSizePixel=0 b.Text=txt b.Font=Enum.Font.GothamBlack b.TextSize=8 b.TextColor3=col b.Visible=false b.ZIndex=20
    co(b,10) stk(b,col,1.5,0.25) safeClick(b,function() b.Visible=false openFn() end) return b
end
...
grabBar.BackgroundColor3=C.BG grabBar.BackgroundTransparency=PANEL_BG_TR grabBar.BorderSizePixel=0
co(grabBar,8) stk(grabBar,C.LIME,1.5,0.2)
-- top accent line
local gTopLine=Instance.new("Frame",grabBar) gTopLine.Size=UDim2.new(1,0,0,2) gTopLine.BackgroundColor3=C.LIME gTopLine.BorderSizePixel=0 co(gTopLine,2)
...
gbTxt.Text="PHANTOM GRAB" gbTxt.Font=Enum.Font.GothamBold gbTxt.TextSize=8 gbTxt.TextColor3=C.LIME gbTxt.ZIndex=2
stealBarTxt=gbTxt

-- ====== PLOT CONTROL BOX + FRIEND BAR (combined, bigger box) ======
-- Single unified box: "PLOT CONTROL" header section + allow friends toggle below
local plotBox=Instance.new("Frame",sg)
plotBox.Size=UDim2.fromOffset(196,58)
plotBox.AnchorPoint=Vector2.new(0.5,0)
plotBox.Position=UDim2.new(0.5,0,0,32)
plotBox.BackgroundColor3=C.PINKD
plotBox.BackgroundTransparency=PANEL_BG_TR
plotBox.BorderSizePixel=0
co(plotBox,10)
local pbStroke=stk(plotBox,C.PINK,1.8,0.3)
makeDrag(plotBox)

-- Gradient background
local pbGrad=Instance.new("UIGradient",plotBox) pbGrad.Rotation=90
pbGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(40,8,30)),ColorSequenceKeypoint.new(1,Color3.fromRGB(12,4,18))})

-- Top accent line
local pbTopLine=Instance.new("Frame",plotBox) pbTopLine.Size=UDim2.new(1,0,0,2) pbTopLine.BackgroundColor3=C.PINK pbTopLine.BorderSizePixel=0 co(pbTopLine,2)

-- "PLOT CONTROL" header section
local pbHeader=Instance.new("Frame",plotBox) pbHeader.Size=UDim2.new(1,0,0,20) pbHeader.Position=UDim2.fromOffset(0,2)
pbHeader.BackgroundTransparency=1 pbHeader.BorderSizePixel=0
local pbTitle=Instance.new("TextLabel",pbHeader) pbTitle.Size=UDim2.new(1,-36,1,0) pbTitle.Position=UDim2.fromOffset(10,0)
pbTitle.BackgroundTransparency=1 pbTitle.Text="PLOT CONTROL"
pbTitle.Font=Enum.Font.GothamBlack pbTitle.TextSize=9
pbTitle.TextColor3=C.PINK pbTitle.TextXAlignment=Enum.TextXAlignment.Left

-- Hotkey button in header
local fbHkBtn=Instance.new("TextButton",pbHeader)
fbHkBtn.Size=UDim2.fromOffset(26,16) fbHkBtn.Position=UDim2.new(1,-30,0.5,-8)
fbHkBtn.BackgroundColor3=C.PINKD fbHkBtn.BackgroundTransparency=0.1 fbHkBtn.BorderSizePixel=0
fbHkBtn.Text="?" fbHkBtn.Font=Enum.Font.GothamBold fbHkBtn.TextSize=7 fbHkBtn.TextColor3=C.PINK fbHkBtn.AutoButtonColor=false
co(fbHkBtn,5) stk(fbHkBtn,C.PINK,1,0.4)
setupHK("FRIEND",fbHkBtn,Enum.KeyCode.Unknown)

-- Divider
local pbDiv=Instance.new("Frame",plotBox) pbDiv.Size=UDim2.new(0.9,0,0,1) pbDiv.Position=UDim2.new(0.05,0,0,23)
pbDiv.BackgroundColor3=C.PINK pbDiv.BackgroundTransparency=0.55 pbDiv.BorderSizePixel=0

-- Bottom section: allow friends toggle
local fbSection=Instance.new("Frame",plotBox) fbSection.Size=UDim2.new(1,0,0,32) fbSection.Position=UDim2.fromOffset(0,26)
fbSection.BackgroundTransparency=1 fbSection.BorderSizePixel=0

-- Lock icon
local fbLock=Instance.new("TextLabel",fbSection)
fbLock.Size=UDim2.fromOffset(24,32) fbLock.Position=UDim2.fromOffset(8,0)
fbLock.BackgroundTransparency=1 fbLock.Text="🔒" fbLock.TextSize=17 fbLock.Font=Enum.Font.GothamBlack

-- Main label
local fbLbl=Instance.new("TextLabel",fbSection)
fbLbl.Size=UDim2.new(1,-34,1,0) fbLbl.Position=UDim2.fromOffset(30,0)
...
-- Invisible click button (full box except hotkey area)
local fbClickBtn=Instance.new("TextButton",plotBox)
fbClickBtn.Size=UDim2.new(1,-32,1,0) fbClickBtn.Position=UDim2.fromOffset(0,0)
fbClickBtn.BackgroundTransparency=1 fbClickBtn.Text="" fbClickBtn.ZIndex=5

local function setFriendBar(state)
    friendOn=state
    if state then
        tw(plotBox,{BackgroundColor3=Color3.fromRGB(6,42,14),BackgroundTransparency=PANEL_BG_TR},0.18)
        pbGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(8,52,18)),ColorSequenceKeypoint.new(1,Color3.fromRGB(4,22,8))})
        TweenService:Create(pbStroke,TweenInfo.new(0.01),{Color=C.GREEN,Transparency=0.1}):Play()
        TweenService:Create(pbStroke,TweenInfo.new(1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.6,Color=C.LIME}):Play()
        pbTopLine.BackgroundColor3=C.GREEN
        pbTitle.TextColor3=C.GREEN
        fbLbl.Text="Allow Friends: ON" fbLock.Text="🔓"
    else
        TweenService:Create(pbStroke,TweenInfo.new(0.01),{Color=C.PINK,Transparency=0.3}):Play()
        tw(plotBox,{BackgroundColor3=C.PINKD,BackgroundTransparency=PANEL_BG_TR},0.18)
        pbGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.fromRGB(40,8,30)),ColorSequenceKeypoint.new(1,Color3.fromRGB(12,4,18))})
        pbTopLine.BackgroundColor3=C.PINK pbTitle.TextColor3=C.PINK
...
safeClick(fbClickBtn,function()
    ...
    -- Only fire ONCE on click if turning ON, no loop
    if friendOn then fireFriendRequestsOnce() end
    task.delay(1.8,function() friendCooldown=false end) -- 1.8 sec delay
end)

-- ====== AP PANEL ======
local APFr,APCnt,APX,APHk=makePanel("AP SPAM",148,UDim2.fromOffset(42,98),C.HOT)
APFr.Visible=false
local APReo=makeReopener("AP",UDim2.fromOffset(42,98),C.HOT,function() APFr.Visible=true end)
...
local apScr=Instance.new("ScrollingFrame",apSFr) apScr.Size=UDim2.new(1,0,1,0) apScr.BackgroundTransparency=1 apScr.BorderSizePixel=0 apScr.ScrollBarThickness=2 apScr.ScrollBarImageColor3=C.HOT apScr.CanvasSize=UDim2.new(0,0,0,0) apScr.AutomaticCanvasSize=Enum.AutomaticSize.Y
...
local function buildAPCards()
    ...
    for i,p in ipairs(Players:GetPlayers()) do
        ...
        local card=Instance.new("Frame",apScr) card.Size=UDim2.new(1,-2,0,28) card.BackgroundColor3=C.CARD2 card.BackgroundTransparency=CARD2_TR card.BorderSizePixel=0 card.LayoutOrder=i co(card,6)
        local cst=stk(card,C.BORD,1,0.55)
        ...
        safeClick(cb,function()
            selectedTarget=cap
            for _,c2 in ipairs(apScr:GetChildren()) do if c2:IsA("Frame") then c2.BackgroundColor3=C.CARD2 c2.BackgroundTransparency=CARD2_TR local s2=c2:FindFirstChildOfClass("UIStroke") if s2 then s2.Color=C.BORD s2.Transparency=0.55 end end end
            card.BackgroundColor3=Color3.fromRGB(45,10,22) card.BackgroundTransparency=0 cst.Color=C.HOT cst.Transparency=0.05
        end)
    end
end
...
local ragBtn=makeBtn(APCnt,"RAGDOLL",C.HOT,4)
local allBtn=makeBtn(APCnt,"ALL CMDS",C.HOT,5)
safeClick(ragBtn,function() local t=getAPTarget() if not t then return end ragBtn.Text="..." ragBtn.TextColor3=C.DIM runCmds(t,{"ragdoll"},false) task.delay(2,function() if ragBtn.Parent then ragBtn.Text="RAGDOLL" ragBtn.TextColor3=C.HOT end end) end)
safeClick(allBtn,function() local t=getAPTarget() if not t then return end allBtn.Text="..." allBtn.TextColor3=C.DIM runCmds(t,{"rocket","inverse","tiny","jumpscare","morph","balloon"},true) task.delay(3,function() if allBtn.Parent then allBtn.Text="ALL CMDS" allBtn.TextColor3=C.HOT end end) end)

-- ====== MINI AP PANEL ======
local MAFr,MACnt,MAX2,MAHk=makePanel("MINI AP",218,UDim2.fromOffset(198,98),C.TEAL)
MAFr.Visible=false
local MAReo=makeReopener("MA",UDim2.fromOffset(198,98),C.TEAL,function() MAFr.Visible=true end)
...
local function buildMiniCards()
    ...
    for i,p in ipairs(Players:GetPlayers()) do
        if p==lp then continue end
        local row=Instance.new("Frame",maScr) row.Size=UDim2.new(1,-2,0,26) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=CARD2_TR row.BorderSizePixel=0 row.LayoutOrder=i co(row,5) stk(row,C.BORD,1,0.6)
...
-- ====== BLOCK PANEL ======
local BKFr,BKCnt,BKX,BKHk=makePanel("BLOCK",148,UDim2.fromOffset(42,205),C.CORAL)
BKFr.Visible=false
local BKReo=makeReopener("BL",UDim2.fromOffset(42,205),C.CORAL,function() BKFr.Visible=true end)
...
local function buildBlockList()
    ...
    for i,p in ipairs(Players:GetPlayers()) do
        if p==lp then continue end
        local row=Instance.new("Frame",bkScr) row.Size=UDim2.new(1,-2,0,24) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=CARD2_TR row.BorderSizePixel=0 row.LayoutOrder=i co(row,5) stk(row,C.BORD,1,0.6)
...
-- ====== RESET PANEL (right, below TP) ======
local RSFr,RSCnt,RSX,RSHk=makePanel("RESET",144,UDim2.new(1,-152,0,98),C.GOLD)
setupHK("RESET",RSHk,Enum.KeyCode.R)
local RSReoBtn=makeReopener("RST",UDim2.new(1,-46,0,98),C.GOLD,function() RSFr.Visible=true end)

-- Position reset below TP dynamically
local function positionReset()
    if not TPFr.Parent or not RSFr.Parent then return end
    local tpPos=TPFr.AbsolutePosition local tpSz=TPFr.AbsoluteSize
    RSFr.Position=UDim2.fromOffset(tpPos.X,tpPos.Y+tpSz.Y+5)
    RSReoBtn.Position=UDim2.fromOffset(tpPos.X+tpSz.X-38,tpPos.Y+tpSz.Y+5)
end
...
local rsStatRow=Instance.new("Frame",RSCnt) rsStatRow.Size=UDim2.new(1,0,0,18) rsStatRow.BackgroundColor3=C.CARD2 rsStatRow.BackgroundTransparency=CARD2_TR rsStatRow.BorderSizePixel=0 rsStatRow.LayoutOrder=3 co(rsStatRow,6) stk(rsStatRow,C.BORD,1,0.6)
...
local function sideBtn(label,col,cb2)
    local f=Instance.new("Frame",sideF) f.Size=UDim2.fromOffset(36,36)
    f.BackgroundColor3=C.CARD f.BackgroundTransparency=CARD_TR f.BorderSizePixel=0 co(f,18)
    local fs2=stk(f,col,1.5,0.25)
    -- glow dot
    local dot=Instance.new("Frame",f) dot.Size=UDim2.fromOffset(5,5) dot.Position=UDim2.new(0.5,-2.5,0,4) dot.BackgroundColor3=col dot.BorderSizePixel=0 co(dot,5)
    TweenService:Create(dot,TweenInfo.new(1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{BackgroundTransparency=0.75}):Play()
    ...
    b.MouseEnter:Connect(function() tw(f,{BackgroundTransparency=0},0.08) tw(fs2,{Transparency=0},0.08) end)
    b.MouseLeave:Connect(function() tw(f,{BackgroundTransparency=CARD_TR},0.08) tw(fs2,{Transparency=0.25},0.08) end)
    safeClick(b,cb2)
end

sideBtn("AP",C.HOT,function()
...
if isMobile then
    local function mobBtn(txt,yOff,col,cb2)
        ...
        f.BackgroundColor3=C.CARD f.BackgroundTransparency=CARD_TR f.BorderSizePixel=0 co(f,14) stk(f,col,1.5,0.25)
...
task.spawn(function()
    while sg.Parent do task.wait(0.4)
        if apSelLbl.Parent then local t=selectedTarget if t and t.Parent then apSelLbl.Text="→ "..t.DisplayName apSelLbl.TextColor3=C.HOT else apSelLbl.Text="tap to target" apSelLbl.TextColor3=C.DIM end end
...
UIS.InputBegan:Connect(function(inp,gpe)
    ...
    if inp.KeyCode==hotkeys.FRIEND then
        ...
        setFriendBar(not friendOn)
        if friendOn then fireFriendRequestsOnce() end
        task.delay(1.8,function() friendCooldown=false end)
    end
end)
