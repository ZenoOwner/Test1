...
pcall(function()
    ...
    local function ps(id,vol)
        ...
        pcall(function() s:Play() end) table.insert(sirenSounds,s) return s
    end
    ...
    local flashing=true
    local flashColors={
        Color3.fromRGB(255,0,0),Color3.fromRGB(15,0,0),Color3.fromRGB(150,0,0),
        Color3.fromRGB(0,0,0),Color3.fromRGB(255,30,0),Color3.fromRGB(5,0,15),
        Color3.fromRGB(80,0,0),Color3.fromRGB(0,0,0)
    }
    task.spawn(function()
        while flashing do
            pcall(function()
    ...
    -- Fade out sirens slowly while "just kidding" shows
    task.spawn(function()
        for i=1,20 do task.wait(0.15)
            for _,s in ipairs(sirenSounds) do pcall(function() s.Volume=math.max(0,s.Volume-0.045) end) end
        end
        for _,s in ipairs(sirenSounds) do pcall(function() s:Stop() s:Destroy() end) end
    end)
    pcall(function() local d=Instance.new("Sound",SS) d.SoundId="rbxassetid://6895079813" d.Volume=0.55 d:Play() game:GetService("Debris"):AddItem(d,4) end)
    ...
    jkLbl.TextSize=50 jkLbl.TextColor3=Color3.fromRGB(120,255,70) jkLbl.TextTransparency=1 jkLbl.ZIndex=3
    ...
    task.wait(2.0) -- total ~8 seconds in loading
...
-- ================================================================
-- COMPLETELY NEW VIVID COLOR SCHEME
-- Deep space glass morphism with maximum contrast neon accents
-- ================================================================
local C={
    -- Panel backgrounds (deep + slight color tint)
    BG      = Color3.fromRGB(4,5,12),
    CARD    = Color3.fromRGB(8,10,20),
    CARD2   = Color3.fromRGB(12,15,28),

    -- Text
    TEXT    = Color3.fromRGB(242,238,255),
    DIM     = Color3.fromRGB(105,112,155),
    WHITE   = Color3.new(1,1,1),

    -- Panel accent colors - each panel gets its own vivid color
    -- AP: Vivid Orange-Red
    AP      = Color3.fromRGB(255,75,0),
    APD     = Color3.fromRGB(40,10,0),

    -- Mini AP: Neon Cyan
    MA      = Color3.fromRGB(0,230,255),
    MAD     = Color3.fromRGB(0,30,42),

    -- Block: Vivid Red
    BK      = Color3.fromRGB(255,30,60),
    BKD     = Color3.fromRGB(42,5,10),

    -- TP: Electric Blue
    TP      = Color3.fromRGB(50,150,255),
    TPD     = Color3.fromRGB(5,18,42),

    -- Reset: Vivid Gold
    RS      = Color3.fromRGB(255,195,0),
    RSD     = Color3.fromRGB(42,30,0),

    -- ESP: Neon Purple
    ESP     = Color3.fromRGB(185,80,255),
    ESPD    = Color3.fromRGB(24,8,42),

    -- Friend/Plot: Hot Magenta
    FR      = Color3.fromRGB(255,50,180),
    FRD     = Color3.fromRGB(42,5,28),

    -- Grab bar: Neon Green
    GRAB    = Color3.fromRGB(80,255,100),

    -- Steal
    ON      = Color3.fromRGB(60,240,80),
    OFF     = Color3.fromRGB(20,22,38),
    GREEN   = Color3.fromRGB(60,240,80),
    RED     = Color3.fromRGB(255,40,55),
    BORD    = Color3.fromRGB(35,42,80),
}
...
local function stk(p,col,t,tr)
    local s=Instance.new("UIStroke",p) s.Color=col or C.BORD s.Thickness=t or 1 s.Transparency=tr or 0.3
    return s
end
...
-- ====== FRIEND REQUEST
-- FIXED: NO loop. Fires once on click ON. Turning OFF is always instant (no cooldown blocks it).
local friendOn=false
local friendFireCooldown=false  -- only blocks re-firing, NOT turning off
...
-- Panel: vivid colored header strip, dark glass body
local function makePanel(name,w,startPos,accent,darkBg)
    ...
    co(frame,11)
    local outerS=stk(frame,accent,1.8,0.4)
    -- diagonal gradient
    ...
    -- Header: full accent-colored strip at top
    local hdr=Instance.new("Frame",frame) hdr.Size=UDim2.new(1,0,0,28)
    hdr.BackgroundColor3=accent hdr.BackgroundTransparency=0.72 hdr.BorderSizePixel=0 hdr.LayoutOrder=1
    co(hdr,11)
    ...
    -- Header top accent line (full brightness)
    local hline=Instance.new("Frame",hdr) hline.Size=UDim2.new(1,0,0,2) hline.BackgroundColor3=accent hline.BorderSizePixel=0 co(hline,2) hline.BackgroundTransparency=0
    -- Title
...
grabBar.BackgroundColor3=C.BG grabBar.BackgroundTransparency=BG_TR grabBar.BorderSizePixel=0
co(grabBar,8)
-- vivid green top line
...
plotBox.BackgroundColor3=C.FRD
plotBox.BackgroundTransparency=BG_TR
plotBox.BorderSizePixel=0 co(plotBox,10)
...
pbTitle.Font=Enum.Font.GothamBlack pbTitle.TextSize=9 pbTitle.TextColor3=C.WHITE
...
safeClick(fbClickBtn,function()
    if friendOn then
        -- Turning OFF: always instant, no cooldown
        setFriendBar(false)
    else
        -- Turning ON: fire once, apply cooldown
        if friendFireCooldown then return end
        friendFireCooldown=true
        setFriendBar(true)
        fireFriendRequestsOnce()
        task.delay(1.8,function() friendFireCooldown=false end)
    end
end)

-- ====== AP PANEL ======
local APFr,APCnt,APX,APHk=makePanel("AP SPAM",148,UDim2.fromOffset(42,100),C.AP)
APFr.Visible=false
local APReo=makeReopener("AP",UDim2.fromOffset(42,100),C.AP,function() APFr.Visible=true end)
...
-- ====== MINI AP PANEL ======
local MAFr,MACnt,MAX2,MAHk=makePanel("MINI AP",218,UDim2.fromOffset(198,100),C.MA)
MAFr.Visible=false
local MAReo=makeReopener("MA",UDim2.fromOffset(198,100),C.MA,function() MAFr.Visible=true end)
...
-- ====== BLOCK PANEL ======
local BKFr,BKCnt,BKX,BKHk=makePanel("BLOCK",148,UDim2.fromOffset(42,208),C.BK)
BKFr.Visible=false
local BKReo=makeReopener("BL",UDim2.fromOffset(42,208),C.BK,function() BKFr.Visible=true end)
...
task.spawn(function() task.wait(0.3) positionReset()
    TPFr:GetPropertyChangedSignal("AbsoluteSize"):Connect(positionReset)
    TPFr:GetPropertyChangedSignal("AbsolutePosition"):Connect(positionReset)
end)
...
local function sideBtn(label,col,cb2)
    ...
    -- inner gradient tint
    local ig=Instance.new("UIGradient",f) ig.Rotation=45
    ig.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),ColorSequenceKeypoint.new(1,Color3.new(0,0,0))})
    ig.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.88),NumberSequenceKeypoint.new(1,0.96)})
    -- pulse dot
    local dot=Instance.new("Frame",f) dot.Size=UDim2.fromOffset(4,4) dot.Position=UDim2.new(0.5,-2,0,4) dot.BackgroundColor3=col dot.BorderSizePixel=0 co(dot,4)
    ...
    local b=Instance.new("TextButton",f) b.Size=UDim2.new(1,0,1,0) b.BackgroundTransparency=1 b.Text=label b.Font=Enum.Font.GothamBlack b.TextSize=10 b.TextColor3=col b.BorderSizePixel=0 b.AutoButtonColor=false b.Position=UDim2.fromOffset(0,3)
...
UIS.InputBegan:Connect(function(inp,gpe)
    ...
    if inp.KeyCode==hotkeys.FRIEND then
        if friendOn then
            ...
            if friendFireCooldown then return end
            friendFireCooldown=true
            setFriendBar(true) fireFriendRequestsOnce()
            task.delay(1.8,function() friendFireCooldown=false end)
        end
    end
end)
