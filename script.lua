-- leaked by yo boi require(kabob) .gg/freesources
print("leaked by yo boi require(kabob) .gg/freesources")
setclipboard("discord.gg/freesources")
...
task.spawn(function()
    ...
    -- â”€â”€ ICE Hub style B&W palette â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    local C = {
    ...
    -- â”€â”€ Helper: make a panel frame â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    local function mkPanel(parent, w, h, x, y, title, closeable)
        local gui = Instance.new("ScreenGui", PlayerGui)
        gui.ResetOnSpawn = false; gui.ZIndexBehavior = Enum.ZIndexBehavior.Global
        local f = Instance.new("Frame", gui)
        f.Size = UDim2.new(0, w, 0, h)
        f.Position = UDim2.new(x, 0, y, 0)
        f.BackgroundColor3 = C.Panel; f.BackgroundTransparency = 0.05; f.BorderSizePixel = 0
        Instance.new("UICorner", f).CornerRadius = UDim.new(0, 10)
        local st = Instance.new("UIStroke", f); st.Color = C.Border; st.Thickness = 1.5; st.Transparency = 0.1
        -- title bar
        local tb = Instance.new("Frame", f); tb.Size = UDim2.new(1,0,0,28); tb.BackgroundColor3 = C.Bg; tb.BorderSizePixel = 0
        Instance.new("UICorner", tb).CornerRadius = UDim.new(0, 10)
        local fix = Instance.new("Frame", tb); fix.Size = UDim2.new(1,0,0,14); fix.Position = UDim2.new(0,0,1,-14)
        fix.BackgroundColor3 = C.Bg; fix.BorderSizePixel = 0
        local tl = Instance.new("TextLabel", tb)
        tl.Size = UDim2.new(1,-30,1,0); tl.Position = UDim2.new(0,10,0,0)
        tl.BackgroundTransparency = 1; tl.Text = title
        tl.Font = Enum.Font.GothamBlack; tl.TextSize = 11; tl.TextColor3 = C.Dim
        tl.TextXAlignment = Enum.TextXAlignment.Left
        MakeDraggable(tb, f)
        if closeable then
            local cb = Instance.new("TextButton", tb)
            cb.Size = UDim2.new(0,20,0,20); cb.Position = UDim2.new(1,-22,0.5,0); cb.AnchorPoint = Vector2.new(0,0.5)
            cb.BackgroundColor3 = Color3.fromRGB(180,60,60); cb.BorderSizePixel = 0; cb.AutoButtonColor = false
            cb.Text = ""; cb.Font = Enum.Font.GothamBlack; cb.TextSize = 10; cb.TextColor3 = C.White
            Instance.new("UICorner", cb).CornerRadius = UDim.new(1,0)
            -- x drawn via two rotated frames
            local x1 = Instance.new("Frame", cb); x1.Size = UDim2.new(0,10,0,2); x1.AnchorPoint = Vector2.new(0.5,0.5); x1.Position = UDim2.new(0.5,0,0.5,0)
            x1.BackgroundColor3 = C.White; x1.BorderSizePixel = 0; x1.Rotation = 45
            Instance.new("UICorner",x1).CornerRadius = UDim.new(1,0)
            local x2 = Instance.new("Frame", cb); x2.Size = UDim2.new(0,10,0,2); x2.AnchorPoint = Vector2.new(0.5,0.5); x2.Position = UDim2.new(0.5,0,0.5,0)
            x2.BackgroundColor3 = C.White; x2.BorderSizePixel = 0; x2.Rotation = -45
            Instance.new("UICorner",x2).CornerRadius = UDim.new(1,0)
            cb.MouseButton1Click:Connect(function() gui.Enabled = false end)
        end
        return gui, f, tb
    end

    -- â”€â”€ Helper: row toggle â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    local function mkToggleRow(parent, yOff, labelTxt, initOn, onChange)
        local row = Instance.new("Frame", parent)
        row.Size = UDim2.new(1,-16,0,28); row.Position = UDim2.new(0,8,0,yOff)
        row.BackgroundTransparency = 1; row.BorderSizePixel = 0
        local lb = Instance.new("TextLabel", row)
        lb.Size = UDim2.new(1,-60,1,0); lb.BackgroundTransparency = 1
        lb.Text = labelTxt; lb.Font = Enum.Font.GothamBold; lb.TextSize = 12
        lb.TextColor3 = C.White; lb.TextXAlignment = Enum.TextXAlignment.Left
        local pill = Instance.new("Frame", row)
        pill.Size = UDim2.new(0,42,0,20); pill.Position = UDim2.new(1,-42,0.5,0); pill.AnchorPoint = Vector2.new(0,0.5)
        pill.BackgroundColor3 = initOn and C.Green or C.Dim; pill.BorderSizePixel = 0
        Instance.new("UICorner", pill).CornerRadius = UDim.new(1,0)
        local knob = Instance.new("Frame", pill)
        knob.Size = UDim2.new(0,15,0,15); knob.AnchorPoint = Vector2.new(0,0.5)
        knob.Position = initOn and UDim2.new(1,-17,0.5,0) or UDim2.new(0,2,0.5,0)
        knob.BackgroundColor3 = Color3.fromRGB(255,255,255); knob.BorderSizePixel = 0
        Instance.new("UICorner", knob).CornerRadius = UDim.new(1,0)
        local on = initOn
        local function setState(s)
            on = s
            TweenService:Create(pill,TweenInfo.new(0.15),{BackgroundColor3=s and C.Green or C.Dim}):Play()
            TweenService:Create(knob,TweenInfo.new(0.15),{Position=s and UDim2.new(1,-17,0.5,0) or UDim2.new(0,2,0.5,0)}):Play()
        end
        local btn = Instance.new("TextButton", row)
        btn.Size = UDim2.new(1,0,1,0); btn.BackgroundTransparency = 1; btn.Text = ""; btn.BorderSizePixel = 0
        btn.MouseButton1Click:Connect(function() on=not on; setState(on); onChange(on) end)
        return setState
    end

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- TOP-LEFT: Server Brainrots list
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local brGuiObj, brFrame, brTB = mkPanel(nil, 165, 220, 0, 0, "NUGGET HUB  Â·  Server Brainrots", false)
    brFrame.Position = UDim2.new(0, 8, 0, 8)
    local brScroll = Instance.new("ScrollingFrame", brFrame)
    brScroll.Size = UDim2.new(1,-4,1,-30); brScroll.Position = UDim2.new(0,2,0,28)
    brScroll.BackgroundTransparency = 1; brScroll.BorderSizePixel = 0
    brScroll.ScrollBarThickness = 3; brScroll.ScrollBarImageColor3 = C.Dim
    brScroll.ScrollBarImageTransparency = 0.4
    local brLayout = Instance.new("UIListLayout", brScroll)
    brLayout.Padding = UDim.new(0,2); brLayout.SortOrder = Enum.SortOrder.LayoutOrder
    brLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        brScroll.CanvasSize = UDim2.new(0,0,0,brLayout.AbsoluteContentSize.Y)
    end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- BOTTOM-LEFT: Movement Panel (Invis + sliders)
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local mvGuiObj, mvFrame, mvTB = mkPanel(nil, 180, 200, 0, 0, "NUGGET HUB  Â·  Movement Panel", true)
    mvFrame.Position = UDim2.new(0, 8, 1, -210)
    -- Invis toggle
    local invisSetState = mkToggleRow(mvFrame, 32, "Invis Steal (U):", false, function(v)
        if _G.toggleInvisibleSteal then pcall(_G.toggleInvisibleSteal) end
    end)
    -- Walkspeed toggle
    mkToggleRow(mvFrame, 64, "Walkspeed:", false, function(v)
        -- placeholder
    end)
    -- Rotation slider label
    local function mkSliderRow(parent, yOff, labelTxt, minV, maxV, defV, onCh)
        local lbl = Instance.new("TextLabel", parent)
        lbl.Size = UDim2.new(1,-16,0,16); lbl.Position = UDim2.new(0,8,0,yOff)
        lbl.BackgroundTransparency = 1; lbl.Text = labelTxt..": "..tostring(defV)
        lbl.Font = Enum.Font.GothamMedium; lbl.TextSize = 10; lbl.TextColor3 = C.Gray; lbl.TextXAlignment = Enum.TextXAlignment.Left
        local bg = Instance.new("Frame", parent)
        bg.Size = UDim2.new(1,-16,0,6); bg.Position = UDim2.new(0,8,0,yOff+18)
        ...
        knob.Position = UDim2.new(pct,0,0.5,0); knob.BackgroundColor3 = C.White; knob.BorderSizePixel = 0
        Instance.new("UICorner",knob).CornerRadius = UDim.new(1,0)
        ...
        UserInputService.InputChanged:Connect(function(i)
            if dragging and i.UserInputType==Enum.UserInputType.MouseMovement then
                ...
                lbl.Text = labelTxt..": "..tostring(v); onCh(v)
            end
        end)
    end
    mkSliderRow(mvFrame, 98,  "Rotation", 0, 360, Config.InvisStealAngle or 233, function(v) Config.InvisStealAngle=v; _G.InvisStealAngle=v end)
    mkSliderRow(mvFrame, 130, "Depth",    0, 10,  Config.SinkSliderValue or 5,   function(v) Config.SinkSliderValue=v; _G.SinkSliderValue=v end)
    mkSliderRow(mvFrame, 162, "Speed",    5, 100, Config.StealSpeed or 20,       function(v) Config.StealSpeed=v; SaveConfig() end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- TOP-RIGHT: Steal Panel
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local stGuiObj, stFrame, stTB = mkPanel(nil, 190, 260, 1, 0, "NUGGET HUB  Â·  Steal Panel", false)
    stFrame.Position = UDim2.new(1, -198, 0, 8)
    -- Toggle rows
    local setStealHighest  = mkToggleRow(stFrame, 32,  "Steal Highest:",  Config.StealHighest  or false, function(v) Config.StealHighest=v; SaveConfig() end)
    local setStealPriority = mkToggleRow(stFrame, 64,  "Steal Priority:", Config.StealPriority or false, function(v) Config.StealPriority=v; SaveConfig() end)
    local setStealNearest  = mkToggleRow(stFrame, 96,  "Steal Nearest:",  Config.StealNearest  or false, function(v) Config.StealNearest=v; SaveConfig() end)
    local setAutoTurret    = mkToggleRow(stFrame, 128, "Auto Turret (G):", false, function(v)
        Config.AutoDestroyTurrets=v; SaveConfig()
    end)
    local setAutoKick      = mkToggleRow(stFrame, 160, "Auto Kick:",      Config.AutoKickOnSteal or false, function(v) Config.AutoKickOnSteal=v; SaveConfig() end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- BOTTOM-RIGHT: Actions Panel
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local acGuiObj, acFrame, acTB = mkPanel(nil, 180, 320, 1, 0, "NUGGET HUB  Â·  Actions", false)
    acFrame.Position = UDim2.new(1, -188, 1, -330)
    local function mkActionBtn(parent, yOff, labelTxt, bgCol, onClick)
        local btn = Instance.new("TextButton", parent)
        btn.Size = UDim2.new(1,-16,0,36); btn.Position = UDim2.new(0,8,0,yOff)
        btn.BackgroundColor3 = bgCol; btn.BorderSizePixel = 0; btn.AutoButtonColor = false
        btn.Text = labelTxt; btn.Font = Enum.Font.GothamBold; btn.TextSize = 13; btn.TextColor3 = C.White
        Instance.new("UICorner", btn).CornerRadius = UDim.new(0,8)
        local st = Instance.new("UIStroke",btn); st.Color = C.Border; st.Thickness=1; st.Transparency=0.4
        btn.MouseEnter:Connect(function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=Color3.fromRGB(bgCol.R*255+15,bgCol.G*255+15,bgCol.B*255+15)}):Play() end)
        btn.MouseLeave:Connect(function() TweenService:Create(btn,TweenInfo.new(0.1),{BackgroundColor3=bgCol}):Play() end)
        btn.MouseButton1Click:Connect(onClick)
        return btn
    end
    mkActionBtn(acFrame, 32,  "Teleport (T)",    Color3.fromRGB(22,22,30), function() pcall(runAutoSnipe) end)
    mkActionBtn(acFrame, 74,  "Insta Cloner",    Color3.fromRGB(22,22,30), function() pcall(instantClone) end)
    local carpetBtnRef = mkActionBtn(acFrame, 116, "Carpet Speed: OFF", Color3.fromRGB(22,22,30), function(self)
        carpetSpeedEnabled = not carpetSpeedEnabled
        setCarpetSpeed(carpetSpeedEnabled)
    end)
    mkActionBtn(acFrame, 158, "Rejoin Server",   Color3.fromRGB(22,22,30), function()
        pcall(function() game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer) end)
    end)
    mkActionBtn(acFrame, 200, "Server Hop",      Color3.fromRGB(22,22,30), function()
        ShowNotification("SERVER HOP","Not implemented")
    end)
    mkActionBtn(acFrame, 242, "Reset",           Color3.fromRGB(140,30,30), function() pcall(executeReset) end)
    -- Settings button opens settings modal
    local settingsActionBtn = mkActionBtn(acFrame, 280, "Settings", Color3.fromRGB(40,80,140), function() end)

    -- Update carpet speed button text
    task.spawn(function()
        while carpetBtnRef.Parent do
            carpetBtnRef.Text = "Carpet Speed: " .. (State and State.carpetSpeedEnabled and "ON" or "OFF")
            task.wait(0.5)
        end
    end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- CENTER HUD (logo + name + status)
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local hudGui = Instance.new("ScreenGui", PlayerGui)
    hudGui.Name = "NuggetHubHUD"; hudGui.ResetOnSpawn = false

    local hudFrame = Instance.new("Frame", hudGui)
    hudFrame.Size = UDim2.new(0, 300, 0, 130)
    hudFrame.AnchorPoint = Vector2.new(0.5, 1)
    hudFrame.Position = UDim2.new(0.5, 0, 1, -10)
    hudFrame.BackgroundColor3 = C.Panel; hudFrame.BackgroundTransparency = 0.15; hudFrame.BorderSizePixel = 0
    Instance.new("UICorner", hudFrame).CornerRadius = UDim.new(0, 16)
    local hudStroke = Instance.new("UIStroke", hudFrame)
    hudStroke.Color = C.Border; hudStroke.Thickness = 1.5; hudStroke.Transparency = 0.05
    -- rotating stroke
    ...
    task.spawn(function()
        while hudStroke.Parent do
            hudStrokeGrad.Rotation = (hudStrokeGrad.Rotation + 0.5) % 360
            task.wait(0.05)
        end
    end)
    MakeDraggable(hudFrame, hudFrame)

    -- Logo circle (Nugget Hub art styled in B&W)
    local logoCircle = Instance.new("Frame", hudFrame)
    logoCircle.Size = UDim2.new(0, 64, 0, 64); logoCircle.AnchorPoint = Vector2.new(0.5, 0)
    logoCircle.Position = UDim2.new(0.5, 0, 0, -32)
    logoCircle.BackgroundColor3 = Color3.fromRGB(0,0,0); logoCircle.BorderSizePixel = 0
    Instance.new("UICorner", logoCircle).CornerRadius = UDim.new(1,0)
    local lcStroke = Instance.new("UIStroke", logoCircle)
    lcStroke.Color = Color3.fromRGB(230,230,230); lcStroke.Thickness = 2.5; lcStroke.Transparency = 0.1
    task.spawn(function()
        while lcStroke.Parent do
            TweenService:Create(lcStroke,TweenInfo.new(1.5,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Transparency=0}):Play()
            task.wait(1.5)
            TweenService:Create(lcStroke,TweenInfo.new(1.5,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Transparency=0.5}):Play()
            task.wait(1.5)
        end
    end)
    -- Nugget/Hub text inside circle
    local logoN = Instance.new("TextLabel", logoCircle)
    logoN.Size=UDim2.new(1,0,0,34); logoN.Position=UDim2.new(0,0,0,4)
    logoN.BackgroundTransparency=1; logoN.Text="Nugget"
    logoN.Font=Enum.Font.GothamBlack; logoN.TextSize=15; logoN.TextColor3=Color3.fromRGB(240,240,240)
    local logoH = Instance.new("TextLabel", logoCircle)
    logoH.Size=UDim2.new(1,0,0,26); logoH.Position=UDim2.new(0,0,0,36)
    logoH.BackgroundTransparency=1; logoH.Text="Hub"
    logoH.Font=Enum.Font.GothamBlack; logoH.TextSize=12; logoH.TextColor3=Color3.fromRGB(180,180,180)

    -- Hub name
    local hudName = Instance.new("TextLabel", hudFrame)
    hudName.Size=UDim2.new(1,-16,0,28); hudName.Position=UDim2.new(0,8,0,38)
    hudName.BackgroundTransparency=1; hudName.Text="NUGGET HUB"
    hudName.Font=Enum.Font.GothamBlack; hudName.TextSize=22; hudName.TextColor3=C.White
    hudName.TextXAlignment=Enum.TextXAlignment.Center
    hudName.TextStrokeTransparency=0.7; hudName.TextStrokeColor3=Color3.fromRGB(255,255,255)
    -- shimmer on hub name
    local nameShimmer = Instance.new("UIGradient", hudName)
    nameShimmer.Color=ColorSequence.new{
        ColorSequenceKeypoint.new(0,Color3.fromRGB(160,160,160)),
        ColorSequenceKeypoint.new(0.45,Color3.fromRGB(160,160,160)),
        ColorSequenceKeypoint.new(0.5,Color3.fromRGB(255,255,255)),
        ColorSequenceKeypoint.new(0.55,Color3.fromRGB(160,160,160)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(160,160,160)),
    }
    nameShimmer.Rotation=30; nameShimmer.Offset=Vector2.new(-1.5,0)
    task.spawn(function()
        while hudName.Parent do
            task.wait(5); nameShimmer.Offset=Vector2.new(-1.5,0)
            local tw=TweenService:Create(nameShimmer,TweenInfo.new(1.5,Enum.EasingStyle.Linear),{Offset=Vector2.new(1.5,0)})
            tw:Play(); tw.Completed:Wait()
        end
    end)

    -- Status label
    local hudStatus = Instance.new("TextLabel", hudFrame)
    hudStatus.Size=UDim2.new(1,-16,0,18); hudStatus.Position=UDim2.new(0,8,0,66)
    hudStatus.BackgroundTransparency=1; hudStatus.Text="READY"
    hudStatus.Font=Enum.Font.GothamBold; hudStatus.TextSize=13; hudStatus.TextColor3=C.Green
    hudStatus.TextXAlignment=Enum.TextXAlignment.Center

    -- Target bar
    local tgtBg = Instance.new("Frame", hudFrame)
    tgtBg.Size=UDim2.new(1,-16,0,22); tgtBg.Position=UDim2.new(0,8,0,88)
    tgtBg.BackgroundColor3=C.Card; tgtBg.BorderSizePixel=0
    Instance.new("UICorner",tgtBg).CornerRadius=UDim.new(0,6)
    local targetLblHUD=Instance.new("TextLabel",tgtBg)
    targetLblHUD.Size=UDim2.new(1,-8,1,0); targetLblHUD.Position=UDim2.new(0,6,0,0)
    targetLblHUD.BackgroundTransparency=1; targetLblHUD.Text="Searching..."
    targetLblHUD.Font=Enum.Font.GothamBold; targetLblHUD.TextSize=12; targetLblHUD.TextColor3=C.White
    targetLblHUD.TextXAlignment=Enum.TextXAlignment.Left; targetLblHUD.TextTruncate=Enum.TextTruncate.AtEnd

    -- Progress bar under target
    local tgtPgBg=Instance.new("Frame",hudFrame); tgtPgBg.Size=UDim2.new(1,-16,0,4); tgtPgBg.Position=UDim2.new(0,8,0,112)
    tgtPgBg.BackgroundColor3=C.Card; tgtPgBg.BorderSizePixel=0; Instance.new("UICorner",tgtPgBg).CornerRadius=UDim.new(0,2)
    local progressBarFill=Instance.new("Frame",tgtPgBg)
    progressBarFill.Size=UDim2.new(0,0,1,0); progressBarFill.BackgroundColor3=C.Accent; progressBarFill.BorderSizePixel=0
    Instance.new("UICorner",progressBarFill).CornerRadius=UDim.new(0,2)

    -- FPS / PING / made by
    local hudInfo = Instance.new("TextLabel", hudFrame)
    hudInfo.Size=UDim2.new(1,-16,0,14); hudInfo.Position=UDim2.new(0,8,0,118)
    hudInfo.BackgroundTransparency=1; hudInfo.RichText=true
    hudInfo.Font=Enum.Font.GothamMedium; hudInfo.TextSize=10; hudInfo.TextColor3=C.Gray
    hudInfo.TextXAlignment=Enum.TextXAlignment.Center

    local acc2,rate2,lastFps2=0,1,60
    RunService.Heartbeat:Connect(function(dt)
        acc2=acc2+dt; if acc2>=rate2 then lastFps2=math.floor(1/dt); acc2=0 end
        local ping=math.floor(LocalPlayer:GetNetworkPing()*1000)
        local fc=lastFps2>=50 and "rgb(180,180,180)" or "rgb(110,110,110)"
        local pc=ping<100 and "rgb(180,180,180)" or "rgb(110,110,110)"
        hudInfo.Text=string.format("<font color='rgb(80,80,80)'>FPS:</font> <font color='%s'>%d</font>  <font color='rgb(80,80,80)'>PING:</font> <font color='%s'>%dms</font>  <font color='rgb(60,60,60)'>made by LIGHT</font>",fc,lastFps2,pc,ping)
        -- update status
        local stealing = LocalPlayer:GetAttribute("Stealing")
        hudStatus.Text = stealing and "STEALING" or "READY"
        hudStatus.TextColor3 = stealing and Color3.fromRGB(255,200,80) or C.Green
        -- update target hud label
        if SharedState.SelectedPetData then
            targetLblHUD.Text = SharedState.SelectedPetData.petName.." â€” "..(SharedState.SelectedPetData.mpsText or "")
        else
            targetLblHUD.Text = "Searching..."
        end
    end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- SETTINGS MODAL (tabbed, opens from Settings button)
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local settingsModal = Instance.new("ScreenGui", PlayerGui)
    settingsModal.Name = "NuggetSettingsModal"; settingsModal.ResetOnSpawn = false; settingsModal.Enabled = false
    settingsModal.DisplayOrder = 100

    local smOverlay = Instance.new("Frame", settingsModal)
    smOverlay.Size = UDim2.new(1,0,1,0); smOverlay.BackgroundColor3 = Color3.fromRGB(0,0,0)
    smOverlay.BackgroundTransparency = 0.6; smOverlay.BorderSizePixel = 0

    local smFrame = Instance.new("Frame", settingsModal)
    smFrame.Size = UDim2.new(0, 560, 0, 380)
    smFrame.AnchorPoint = Vector2.new(0.5, 0.5); smFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    smFrame.BackgroundColor3 = C.Panel; smFrame.BackgroundTransparency = 0.05; smFrame.BorderSizePixel = 0
    Instance.new("UICorner", smFrame).CornerRadius = UDim.new(0, 14)
    local smStroke = Instance.new("UIStroke", smFrame); smStroke.Color = C.Border; smStroke.Thickness = 1.5; smStroke.Transparency = 0.1

    -- title bar
    local smHdr = Instance.new("Frame", smFrame)
    smHdr.Size=UDim2.new(1,0,0,40); smHdr.BackgroundColor3=C.Bg; smHdr.BorderSizePixel=0
    Instance.new("UICorner",smHdr).CornerRadius=UDim.new(0,14)
    local smFix=Instance.new("Frame",smHdr); smFix.Size=UDim2.new(1,0,0,14); smFix.Position=UDim2.new(0,0,1,-14)
    smFix.BackgroundColor3=C.Bg; smFix.BorderSizePixel=0
    local smTitle=Instance.new("TextLabel",smHdr); smTitle.Size=UDim2.new(1,-50,1,0); smTitle.Position=UDim2.new(0,16,0,0)
    smTitle.BackgroundTransparency=1; smTitle.Text="NUGGET HUB  Â·  Settings"
    smTitle.Font=Enum.Font.GothamBlack; smTitle.TextSize=14; smTitle.TextColor3=C.White; smTitle.TextXAlignment=Enum.TextXAlignment.Left
    local smClose=Instance.new("TextButton",smHdr)
    smClose.Size=UDim2.new(0,24,0,24); smClose.Position=UDim2.new(1,-30,0.5,0); smClose.AnchorPoint=Vector2.new(0,0.5)
    smClose.BackgroundColor3=Color3.fromRGB(160,50,50); smClose.BorderSizePixel=0; smClose.AutoButtonColor=false
    smClose.Text="Ã—"; smClose.Font=Enum.Font.GothamBlack; smClose.TextSize=16; smClose.TextColor3=C.White
    Instance.new("UICorner",smClose).CornerRadius=UDim.new(1,0)
    smClose.MouseButton1Click:Connect(function() settingsModal.Enabled=false end)
    smOverlay.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then settingsModal.Enabled=false end end)
    MakeDraggable(smHdr, smFrame)

    -- Tab bar
    local tabBar=Instance.new("Frame",smFrame); tabBar.Size=UDim2.new(1,-16,0,30); tabBar.Position=UDim2.new(0,8,0,44)
    tabBar.BackgroundColor3=C.Bg; tabBar.BorderSizePixel=0; Instance.new("UICorner",tabBar).CornerRadius=UDim.new(0,8)
    local tabLayout=Instance.new("UIListLayout",tabBar); tabLayout.FillDirection=Enum.FillDirection.Horizontal
    tabLayout.Padding=UDim.new(0,2); tabLayout.VerticalAlignment=Enum.VerticalAlignment.Center
    Instance.new("UIPadding",tabBar).PaddingLeft=UDim.new(0,4)

    -- Content area
    local smContent=Instance.new("Frame",smFrame); smContent.Size=UDim2.new(1,-16,1,-84); smContent.Position=UDim2.new(0,8,0,78)
    smContent.BackgroundTransparency=1; smContent.BorderSizePixel=0; smContent.ClipsDescendants=true

    local smTabs={}; local smActive=nil
    local function makeSMTab(name)
        local btn=Instance.new("TextButton",tabBar)
        btn.Size=UDim2.new(0,70,1,-4); btn.BackgroundColor3=C.Card; btn.BorderSizePixel=0; btn.AutoButtonColor=false
        btn.Text=name; btn.Font=Enum.Font.GothamBold; btn.TextSize=11; btn.TextColor3=C.Dim
        Instance.new("UICorner",btn).CornerRadius=UDim.new(0,6)
        local pane=Instance.new("ScrollingFrame",smContent)
        pane.Size=UDim2.new(1,0,1,0); pane.BackgroundTransparency=1; pane.BorderSizePixel=0
        pane.ScrollBarThickness=4; pane.ScrollBarImageColor3=C.Dim; pane.ScrollBarImageTransparency=0.4
        pane.Visible=false
        local pl=Instance.new("UIListLayout",pane); pl.Padding=UDim.new(0,6); pl.SortOrder=Enum.SortOrder.LayoutOrder
        Instance.new("UIPadding",pane).PaddingTop=UDim.new(0,4)
        pl:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() pane.CanvasSize=UDim2.new(0,0,0,pl.AbsoluteContentSize.Y+8) end)
        local entry={name=name,btn=btn,pane=pane}; table.insert(smTabs,entry)
        btn.MouseButton1Click:Connect(function()
            for _,t in ipairs(smTabs) do
                local sel=(t.name==name)
                t.btn.BackgroundColor3=sel and C.Accent or C.Card
                t.btn.TextColor3=sel and Color3.fromRGB(0,0,0) or C.Dim
                t.btn.Font=sel and Enum.Font.GothamBlack or Enum.Font.GothamBold
                t.pane.Visible=sel
            end
            smActive=name
        end)
    ...
    local function mkSMRow(pane, labelTxt, initOn, onChange)
        local row=Instance.new("Frame",pane); row.Size=UDim2.new(1,-8,0,40); row.BackgroundColor3=C.Card; row.BorderSizePixel=0
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
        local lb=Instance.new("TextLabel",row); lb.Size=UDim2.new(1,-60,1,0); lb.Position=UDim2.new(0,14,0,0)
        lb.BackgroundTransparency=1; lb.Text=labelTxt; lb.Font=Enum.Font.GothamBold; lb.TextSize=12
        lb.TextColor3=C.White; lb.TextXAlignment=Enum.TextXAlignment.Left; lb.TextYAlignment=Enum.TextYAlignment.Center
        local pill=Instance.new("Frame",row); pill.Size=UDim2.new(0,42,0,20); pill.Position=UDim2.new(1,-50,0.5,0); pill.AnchorPoint=Vector2.new(0,0.5)
        pill.BackgroundColor3=initOn and C.Green or C.Dim; pill.BorderSizePixel=0
        Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
        local knob=Instance.new("Frame",pill); knob.Size=UDim2.new(0,15,0,15); knob.AnchorPoint=Vector2.new(0,0.5)
        knob.Position=initOn and UDim2.new(1,-17,0.5,0) or UDim2.new(0,2,0.5,0)
        knob.BackgroundColor3=Color3.fromRGB(255,255,255); knob.BorderSizePixel=0
        Instance.new("UICorner",knob).CornerRadius=UDim.new(1,0)
        local on=initOn
        ...
        btn.MouseButton1Click:Connect(function()
            on=not on
            TweenService:Create(pill,TweenInfo.new(0.15),{BackgroundColor3=on and C.Green or C.Dim}):Play()
            TweenService:Create(knob,TweenInfo.new(0.15),{Position=on and UDim2.new(1,-17,0.5,0) or UDim2.new(0,2,0.5,0)}):Play()
            onChange(on)
        end)
    end
    local function mkSMSec(pane, txt)
        local f=Instance.new("Frame",pane); f.Size=UDim2.new(1,-8,0,22); f.BackgroundTransparency=1; f.BorderSizePixel=0
        local l=Instance.new("TextLabel",f); l.Size=UDim2.new(1,0,1,0); l.BackgroundTransparency=1
        l.Text=txt:upper(); l.Font=Enum.Font.GothamBlack; l.TextSize=10; l.TextColor3=C.Dim; l.TextXAlignment=Enum.TextXAlignment.Left
    end

    -- MAIN tab
    local mainPane,mainPL = makeSMTab("Main")
    mkSMSec(mainPane,"General")
    mkSMRow(mainPane,"FPS Boost",Config.FPSBoost,function(v) setFPSBoost(v) end)
    mkSMRow(mainPane,"X-Ray",Config.XrayEnabled,function(v) Config.XrayEnabled=v; if v then enableXray() else disableXray() end; SaveConfig() end)
    mkSMRow(mainPane,"Tracer",Config.TracerEnabled,function(v) Config.TracerEnabled=v; SaveConfig() end)
    mkSMRow(mainPane,"Line to Base",Config.LineToBase,function(v) Config.LineToBase=v; SaveConfig() end)
    mkSMRow(mainPane,"Auto TP on Load",Config.TpSettings.TpOnLoad,function(v) Config.TpSettings.TpOnLoad=v; SaveConfig() end)
    mkSMRow(mainPane,"Auto Desync on TP",Config.AutoDesync or false,function(v) Config.AutoDesync=v; SaveConfig() end)

    -- KEYBINDS tab
    local kbPane,kbPL = makeSMTab("Keybinds")
    mkSMSec(kbPane,"Movement")
    mkSMRow(kbPane,"Infinite Jump",Config.TpSettings.InfiniteJump,function(v) setInfiniteJump(v) end)
    mkSMRow(kbPane,"Auto Steal Speed",Config.AutoStealSpeed,function(v) Config.AutoStealSpeed=v; SaveConfig() end)
    mkSMRow(kbPane,"Show Steal Speed Panel",Config.ShowStealSpeedPanel,function(v)
        Config.ShowStealSpeedPanel=v; SaveConfig()
        local sg=PlayerGui:FindFirstChild("StealSpeedUI"); if sg then sg.Enabled=v end
    end)

    -- ESP tab
    local espPane,espPL = makeSMTab("ESP")
    mkSMSec(espPane,"ESP Options")
    mkSMRow(espPane,"Player ESP",Config.PlayerESP,function(v) Config.PlayerESP=v; SaveConfig() end)
    mkSMRow(espPane,"Brainrot ESP",Config.BrainrotESP,function(v) Config.BrainrotESP=v; SaveConfig() end)
    mkSMRow(espPane,"Duel Base ESP",Config.DuelBaseESP,function(v) Config.DuelBaseESP=v; SaveConfig() end)
    mkSMRow(espPane,"Subspace Mine ESP",Config.SubspaceMineESP,function(v) Config.SubspaceMineESP=v; SaveConfig() end)

    -- MISC tab
    local miscPane,miscPL = makeSMTab("Misc")
    mkSMSec(miscPane,"Protection")
    mkSMRow(miscPane,"Anti-Ragdoll V1",Config.AntiRagdoll>0,function(v) Config.AntiRagdoll=v and 1 or 0; startAntiRagdoll(Config.AntiRagdoll); SaveConfig() end)
    mkSMRow(miscPane,"Anti-Ragdoll V2",Config.AntiRagdollV2,function(v) Config.AntiRagdollV2=v; startAntiRagdollV2(v); SaveConfig() end)
    mkSMRow(miscPane,"Anti-Bee & Disco",Config.AntiBeeDisco,function(v) Config.AntiBeeDisco=v; SaveConfig() end)
    mkSMRow(miscPane,"Auto Reset on Balloon",Config.AutoResetOnBalloon,function(v) Config.AutoResetOnBalloon=v; SaveConfig() end)
    mkSMSec(miscPane,"Automation")
    mkSMRow(miscPane,"Auto Invis During Steal",Config.AutoInvisDuringSteal,function(v) Config.AutoInvisDuringSteal=v; _G.AutoInvisDuringSteal=v; SaveConfig() end)
    mkSMRow(miscPane,"Auto Unlock on Steal",Config.AutoUnlockOnSteal,function(v) Config.AutoUnlockOnSteal=v; SaveConfig() end)

    -- UI tab
    local uiPane,uiPL = makeSMTab("UI")
    mkSMSec(uiPane,"Interface")
    mkSMRow(uiPane,"Lock UI Dragging",Config.UILocked,function(v) Config.UILocked=v; SaveConfig() end)
    mkSMRow(uiPane,"Show Unlock Buttons HUD",Config.ShowUnlockButtonsHUD,function(v) Config.ShowUnlockButtonsHUD=v; SaveConfig() end)
    mkSMRow(uiPane,"Click To AP",Config.ClickToAP,function(v) Config.ClickToAP=v; SaveConfig() end)

    -- Priority tab
    local prioPane,prioPL = makeSMTab("Priority")
    mkSMSec(prioPane,"Priority List")
    local custBtn3=Instance.new("TextButton",prioPane); custBtn3.Size=UDim2.new(1,-8,0,40); custBtn3.BackgroundColor3=Color3.fromRGB(30,30,40); custBtn3.BorderSizePixel=0; custBtn3.AutoButtonColor=false
    custBtn3.Text="Open Priority List Customizer"; custBtn3.Font=Enum.Font.GothamBold; custBtn3.TextSize=12; custBtn3.TextColor3=C.White
    Instance.new("UICorner",custBtn3).CornerRadius=UDim.new(0,8)
    custBtn3.MouseButton1Click:Connect(function()
        settingsModal.Enabled=false
        local pg=PlayerGui:FindFirstChild("PriorityListGUI"); if pg then pg.Enabled=not pg.Enabled end
    end)
    -- default to Main tab
    smTabs[1].btn.BackgroundColor3=C.Accent; smTabs[1].btn.TextColor3=Color3.fromRGB(0,0,0); smTabs[1].btn.Font=Enum.Font.GothamBlack; smTabs[1].pane.Visible=true; smActive="Main"

    settingsActionBtn.MouseButton1Click:Connect(function() settingsModal.Enabled=true end)

    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    -- Brainrot list + main hub list plumbing
    -- â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
    local targetLabel = Instance.new("TextLabel") -- dummy so original code compiles
    targetLabel.Parent = hudFrame; targetLabel.Visible = false
    local listFrame = Instance.new("ScrollingFrame") -- dummy
    listFrame.Parent = hudFrame; listFrame.Visible = false; listFrame.Size = UDim2.new(0,0,0,0)
    local uiListLayout = Instance.new("UIListLayout", listFrame)
    local petButtons = {}

    local function updateUI(enabled, allPets)
        -- update brainrot list panel
        for _,c in ipairs(brScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
        if allPets then
            for i,pd in ipairs(allPets) do
                if i > 20 then break end
                local row=Instance.new("Frame",brScroll)
                row.Size=UDim2.new(1,0,0,24); row.BackgroundColor3=(i==1) and C.Card or Color3.fromRGB(0,0,0); row.BackgroundTransparency=0.3; row.BorderSizePixel=0
                local rankL=Instance.new("TextLabel",row); rankL.Size=UDim2.new(0,22,1,0); rankL.Position=UDim2.new(0,4,0,0)
                rankL.BackgroundTransparency=1; rankL.Text="#"..i; rankL.Font=Enum.Font.GothamBold; rankL.TextSize=10
                rankL.TextColor3=i==1 and C.Accent or C.Dim; rankL.TextYAlignment=Enum.TextYAlignment.Center
                local nameL=Instance.new("TextLabel",row); nameL.Size=UDim2.new(1,-80,1,0); nameL.Position=UDim2.new(0,26,0,0)
                nameL.BackgroundTransparency=1; nameL.Text=pd.petName; nameL.Font=Enum.Font.GothamBold; nameL.TextSize=10
                nameL.TextColor3=i==1 and C.White or C.Gray; nameL.TextXAlignment=Enum.TextXAlignment.Left; nameL.TextTruncate=Enum.TextTruncate.AtEnd; nameL.TextYAlignment=Enum.TextYAlignment.Center
                local genL=Instance.new("TextLabel",row); genL.Size=UDim2.new(0,70,1,0); genL.Position=UDim2.new(1,-72,0,0)
                genL.BackgroundTransparency=1; genL.Text=pd.mpsText; genL.Font=Enum.Font.GothamBold; genL.TextSize=10
                genL.TextColor3=C.Dim; genL.TextXAlignment=Enum.TextXAlignment.Right; genL.TextYAlignment=Enum.TextYAlignment.Center
                local btn=Instance.new("TextButton",row); btn.Size=UDim2.new(1,0,1,0); btn.BackgroundTransparency=1; btn.Text=""; btn.BorderSizePixel=0
                btn.MouseButton1Click:Connect(function()
                    selectedTargetIndex=i; selectedTargetUID=pd.uid
                    stealNearestEnabled=false; stealHighestEnabled=false; stealPriorityEnabled=false
                    Config.StealNearest=false; Config.StealHighest=false; Config.StealPriority=false; SaveConfig()
                    SharedState.ListNeedsRedraw=false; updateUI(autoStealEnabled, get_all_pets())
                end)
            end
        end
        SharedState.SelectedPetData = allPets and allPets[selectedTargetIndex or 1]
        listFrame.CanvasSize=UDim2.new(0,0,0,0)
    end

    uiListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() end)
    SharedState.UpdateAutoStealUI=function() updateUI(autoStealEnabled, get_all_pets()) end
    SharedState.EnableBtn=Instance.new("TextButton") -- dummy refs so steal logic doesn't nil error
    SharedState.NearestBtn=Instance.new("TextButton")
    SharedState.HighestBtn=Instance.new("TextButton")
    SharedState.PriorityBtn=Instance.new("TextButton")
    local instantStealBtn=Instance.new("TextButton")
    local customizePriorityBtn=custBtn3
    SharedState.InstantStealBtn=instantStealBtn

    -- stub enable button logic (steal always on)
    local autoStealEnabledLocal = true
    local function enableBtn_Click() end
    local function nearestBtn_Click() end
    local function highestBtn_Click() end
    local function priorityBtn_Click() end
...
CreateToggleSwitch(rShowUnlockHUD, Config.ShowUnlockButtonsHUD, function(ns, set)
    ...
    local hudGui = PlayerGui:FindFirstChild("NuggetStatusHUD")
    if hudGui then
...
task.spawn(function()
    ...
    if PlayerGui:FindFirstChild("NuggetStatusHUD") then PlayerGui.XiStatusHUD:Destroy() end

    local HTheme = {
        Background = Color3.fromRGB(15,17,22),
        Accent1 = Color3.fromRGB(210,210,215),
        Accent2 = Color3.fromRGB(160,160,168),
    ...
    task.spawn(function()
        while stroke.Parent do
            strokeGrad.Rotation = strokeGrad.Rotation + 0.4
            task.wait(0.05)
        end
    end)
    ...
    if privateBuild then
        title.Text = "NUGGET HUB PRIVATE"
    else
        title.Text = "NUGGET HUB"
    end
    title.Font = Enum.Font.GothamBlack
