...
local function save()
    pcall(function() if writefile then writefile(CFG,HttpService:JSONEncode(Cfg)) end end)
end

for _,v in ipairs(CoreGui:GetChildren()) do
    if v:IsA("ScreenGui") and (v.Name:sub(1,4)=="Aeth" or v.Name:sub(1,4)=="PhFV") then v:Destroy() end
end
local SG=Instance.new("ScreenGui"); SG.Name="AethV11"
...
local function co(p,r)
    local c=Instance.new("UICorner",p); c.CornerRadius=UDim.new(0,r or 7)
end
...
local function lb(par,props)
    ...
    l.TextColor3=T.white; l.BorderSizePixel=0
...
local function btn(par,text,sz,pos,bg)
    ...
    b.BackgroundColor3=bg or T.surf; b.BackgroundTransparency=0.22; b.Text=text
    b.TextColor3=T.white; b.Font=Enum.Font.GothamBold; b.TextSize=9
    b.BorderSizePixel=0; b.AutoButtonColor=false; b.Parent=par
    co(b,5); ms(b,1,T.str,0.44); return b
end
local function rotS(f,col)
    ...
    task.spawn(function()
        while s and s.Parent do
            g.Rotation=(g.Rotation+0.9)%360; task.wait(0.016)
        end
    end)
end
local function particles(parent,count)
    for i=1,(count or 4) do
        local p=Instance.new("Frame",parent); p.BackgroundColor3=Color3.fromRGB(200,210,255)
        p.BackgroundTransparency=0.48; p.BorderSizePixel=0; p.ZIndex=0
        local sz=math.random(2,4); p.Size=UDim2.fromOffset(sz,sz)
        ...
        task.spawn(function()
            while p and p.Parent do
                ...
                if p and p.Parent then
                    p.BackgroundTransparency=0.48; sx=math.random()
...
local function mkP(defX,defY,w,h,title,ac,posKey)
    ...
    f.BackgroundColor3=T.panel; f.BackgroundTransparency=0.14; f.BorderSizePixel=0
    f.ClipsDescendants=true; f.Parent=SG; co(f,9); rotS(f,ac or T.a1); particles(f,4)
    local hdr=Instance.new("Frame",f); hdr.Size=UDim2.new(1,0,0,21)
    hdr.BackgroundColor3=T.surf; hdr.BackgroundTransparency=0.18; hdr.BorderSizePixel=0; co(hdr,9)
    local flat=Instance.new("Frame",hdr); flat.Size=UDim2.new(1,0,0,7)
    flat.Position=UDim2.new(0,0,1,-7); flat.BackgroundColor3=T.surf; flat.BackgroundTransparency=0.18; flat.BorderSizePixel=0
    local sep=Instance.new("Frame",f); sep.Size=UDim2.new(1,-8,0,1); sep.Position=UDim2.fromOffset(4,21)
    sep.BackgroundColor3=T.str; sep.BackgroundTransparency=0.68; sep.BorderSizePixel=0
    if title then
    ...
    mb.TextSize=8; mb.TextColor3=T.dim; mb.BackgroundTransparency=0.6
    dragSave(hdr,f,posKey); return f,hdr,mb
end
local function mkF(parent,hH,cH,mb)
    local content=Instance.new("Frame"); content.Size=UDim2.new(1,0,0,cH)
    content.Position=UDim2.fromOffset(0,hH); content.BackgroundTransparency=1
    content.BorderSizePixel=0; content.Parent=parent
...
local function pillR(parent,label,yPos,onCol)
    ...
    local track=Instance.new("Frame",row); track.Size=UDim2.fromOffset(25,12)
    track.Position=UDim2.new(1,-29,0.5,-6); track.BackgroundColor3=T.surf2; track.BorderSizePixel=0; co(track,12)
    local knob=Instance.new("Frame",track); knob.Size=UDim2.fromOffset(8,8)
    knob.Position=UDim2.new(0,2,0.5,-4); knob.BackgroundColor3=T.a2; knob.BorderSizePixel=0; co(knob,8)
...
local function numRow(parent,label,val,mn,mx,yPos,w,onChange)
    local row=Instance.new("Frame"); row.Size=UDim2.fromOffset(w-10,20); row.Position=UDim2.fromOffset(5,yPos)
    row.BackgroundColor3=T.surf; row.BackgroundTransparency=0.28; row.BorderSizePixel=0; row.Parent=parent
    co(row,5); ms(row,1,T.str,0.5)
    lb(row,{Position=UDim2.new(0,6,0,0),Size=UDim2.new(0.46,0,1,0),Text=label,TextSize=8,
        TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
    local mB=Instance.new("TextButton",row); mB.Size=UDim2.fromOffset(14,14); mB.Position=UDim2.new(0.5,0,0.5,-7)
    mB.BackgroundColor3=T.surf2; mB.Text="-"; mB.Font=Enum.Font.GothamBold; mB.TextSize=10
    mB.TextColor3=T.a1; mB.BorderSizePixel=0; mB.AutoButtonColor=false; co(mB,3)
    local vB=Instance.new("TextBox",row); vB.Size=UDim2.fromOffset(26,14); vB.Position=UDim2.new(0.5,16,0.5,-7)
    vB.BackgroundColor3=T.surf2; vB.BorderSizePixel=0; vB.Text=tostring(val)
    vB.Font=Enum.Font.GothamBold; vB.TextSize=9; vB.TextColor3=T.a1; vB.ClearTextOnFocus=false; co(vB,3)
    local pB=Instance.new("TextButton",row); pB.Size=UDim2.fromOffset(14,14); pB.Position=UDim2.new(0.5,44,0.5,-7)
    pB.BackgroundColor3=T.surf2; pB.Text="+"; pB.Font=Enum.Font.GothamBold; pB.TextSize=10
    pB.TextColor3=T.a1; pB.BorderSizePixel=0; pB.AutoButtonColor=false; co(pB,3)
    local cur=val
    local function setV(v)
        cur=math.clamp(math.floor(v),mn,mx); vB.Text=tostring(cur)
...
-- Background systems (once per session)
if not _G._AethBGv11 then
    ...
    -- Anti-bee (FIXED: does NOT scan for "bee" in names = no beehive tool deletion)
    RunService:BindToRenderStep("AethAntiBee",Enum.RenderPriority.Input.Value+1,function()
        ...
        -- Only remove specific effect values, never tools
        for _,obj in ipairs(char:GetDescendants()) do
            if not obj:IsA("Tool") and not obj:IsA("Model") then
                local nl=obj.Name:lower()
                if nl=="invertmovement" or nl=="reverseinput" or nl=="confuse" then
                    pcall(function() obj:Destroy() end)
                end
            end
        end
        -- Correct velocity only if very clearly inverted
    ...
    -- Anti-sentry v2: 3s deployment delay, skip during steal, only move primary part
        ...
        for _,plot in ipairs(plots:GetChildren()) do
            ...
            if tl2 and mp then
                ...
                if not e or not e.bb.Parent then
                    ...
                    local bb=Instance.new("BillboardGui"); bb.Size=UDim2.fromOffset(52,14)
                    bb.StudsOffset=Vector3.new(0,7,0); bb.AlwaysOnTop=true; bb.Adornee=mp
                    bb.MaxDistance=1200; bb.Parent=plot
                    local bgT=Instance.new("Frame",bb); bgT.Size=UDim2.new(1,0,1,0)
                    bgT.BackgroundColor3=T.bg; bgT.BackgroundTransparency=0.18; bgT.BorderSizePixel=0; co(bgT,4)
                    ...
                    local tl=Instance.new("TextLabel",bgT); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1
                    tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=T.yel; tl.TextStrokeTransparency=0.45
                ...
                if m and s2 then
                    ...
                    if myPlotRef and plot==myPlotRef then
                        if tot<=5 and not baseAlerted then
                            baseAlerted=true
                            pcall(function()
                                local s3=Instance.new("Sound"); s3.SoundId="rbxassetid://9118633772"
                                s3.Volume=0.85; s3.Parent=workspace; s3:Play(); Debris:AddItem(s3,5)
                            end)
                        elseif tot>5 then
                            baseAlerted=false
                        end
...
-- Player ESP: SelectionBox outline (white) + BillboardGui showing name + held item
...
local function mkESP(plr)
    ...
    -- White selection box around character
    local sel=Instance.new("SelectionBox"); sel.Name=ESPTAG
    sel.Adornee=char; sel.Color3=T.white; sel.LineThickness=0.06
    sel.SurfaceTransparency=0.88; sel.SurfaceColor3=T.white; sel.Parent=char
    -- BillboardGui above head
    local bb=Instance.new("BillboardGui",char); bb.Name=ESPTAG.."N"
    bb.Adornee=char:FindFirstChild("Head") or hrp; bb.Size=UDim2.fromOffset(130,36)
    bb.StudsOffset=Vector3.new(0,3.5,0); bb.AlwaysOnTop=true
    local bgE=Instance.new("Frame",bb); bgE.Size=UDim2.new(1,0,1,0)
    bgE.BackgroundColor3=T.bg; bgE.BackgroundTransparency=0.22; bgE.BorderSizePixel=0; co(bgE,5)
    ...
    local nameLbl=lb(bgE,{Size=UDim2.new(1,0,0.55,0),BackgroundTransparency=1,Text=plr.DisplayName,
        Font=Enum.Font.GothamBold,TextSize=9,TextColor3=T.white,TextStrokeTransparency=0.45})
    local heldLbl=lb(bgE,{Size=UDim2.new(1,0,0.45,0),Position=UDim2.new(0,0,0.55,0),BackgroundTransparency=1,
        Text="",Font=Enum.Font.Gotham,TextSize=7,TextColor3=T.dim})
    -- Update held item
    task.spawn(function()
        while bb.Parent do
            task.wait(0.5)
...
local function addMineESP(child)
    ...
    local bb=Instance.new("BillboardGui"); bb.Size=UDim2.fromOffset(80,20)
    bb.StudsOffset=Vector3.new(0,4,0); bb.AlwaysOnTop=true; bb.Adornee=part; bb.Parent=workspace
    local bgM=Instance.new("Frame",bb); bgM.Size=UDim2.new(1,0,1,0)
    bgM.BackgroundColor3=Color3.fromRGB(22,4,4); bgM.BackgroundTransparency=0.18; bgM.BorderSizePixel=0; co(bgM,4)
    ...
    local tl=Instance.new("TextLabel",bgM); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1
    tl.Text="MINE: "..child.Name; tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=T.red
...
local function addBrainrotESP(model,plot)
    ...
    if plot then
        ...
        if sign then local tl=sign:FindFirstChild("TextLabel",true); if tl then owner=tl.Text end end
    end
    local bb=Instance.new("BillboardGui"); bb.Name="AethBrainESP"
    bb.Size=UDim2.fromOffset(112,26); bb.StudsOffset=Vector3.new(0,5,0); bb.AlwaysOnTop=true
    bb.Adornee=part; bb.Parent=workspace
    local bg=Instance.new("Frame",bb); bg.Size=UDim2.new(1,0,1,0)
    bg.BackgroundColor3=T.bg; bg.BackgroundTransparency=0.22; bg.BorderSizePixel=0; co(bg,5)
    ...
    model.AncestryChanged:Connect(function()
        if not model.Parent then pcall(function() bb:Destroy() end); brainrotESPCache[model]=nil end
    end)
end
...
local function buildFriendBB(main,isOpen)
    local bb=Instance.new("BillboardGui"); bb.Adornee=main; bb.Size=UDim2.fromOffset(52,18)
    bb.StudsOffset=Vector3.new(0,5,0); bb.AlwaysOnTop=true; bb.Parent=main
    local bg=Instance.new("Frame",bb); bg.Size=UDim2.new(1,0,1,0)
    bg.BackgroundColor3=T.bg; bg.BackgroundTransparency=0.22; bg.BorderSizePixel=0; co(bg,5)
    local bs=Instance.new("UIStroke",bg); bs.Thickness=1
    bs.Color=isOpen and T.grn or T.red; bs.Transparency=0.22; bs.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
...
-- Brainrot Transparency (from leaked script, LocalTransparencyModifier = undetectable, auto 77%)
local BRAIN_TRANS=0.77; local trackedBrainParts={}; local brainTransOn=false
local brainrotNames={}
task.spawn(function()
    pcall(function()
        local models=RS:WaitForChild("Models",5)
        local animals=models and models:WaitForChild("Animals",5)
        if animals then
            for _,m in pairs(animals:GetChildren()) do brainrotNames[m.Name]=true end
        end
    end)
end)
local function trackBrainrotPart(obj)
    if not (obj:IsA("BasePart") or obj:IsA("Decal")) then return end
    local next=obj.Parent
    while next and next~=workspace do
        if brainrotNames[next.Name] then trackedBrainParts[obj]=true; return end
        next=next.Parent
    end
end
RunService.RenderStepped:Connect(function()
    if not brainTransOn then return end
    for obj in pairs(trackedBrainParts) do
        if obj and obj.Parent then
            obj.LocalTransparencyModifier=BRAIN_TRANS
        else
            trackedBrainParts[obj]=nil
        end
    end
end)
local function enableBrainTrans()
    if brainTransOn then return end; brainTransOn=true
    for _,obj in ipairs(workspace:GetDescendants()) do trackBrainrotPart(obj) end
    workspace.DescendantAdded:Connect(function(obj)
        if brainTransOn then trackBrainrotPart(obj) end
    end)
end
...
-- Base Xray (uses LocalTransparencyModifier = server-undetectable)
...
local function fireCmd(tgt,cmd)
    ...
    aFire(tgt,cmd); rowCdEnd[cmd]=tick()+(ROW_CD[cmd] or 30)
end
...
local function buildLook(bl)
    local h2=flattenV(bl); local fs=math.sqrt(math.max(0,1-desiredY^2))
...
-- EXT steal callbacks (Phantom Hub V5 exact)
local StealCBCache={}
...
-- FLASH TP (Phantom Hub V5 exact + RF bypass)
local flashEnabled=Cfg.flashOn; local sliderValue=Cfg.triggerChance/100
local activePrompts={}; local lagOnSteal=false
            ...
            if shop then
                for _,obj in ipairs(shop:GetDescendants()) do
                    if obj.Name:lower():find("admin") then
                        local id=obj:GetAttribute("GamepassId") or obj:GetAttribute("GamePassId")
                        if id and type(id)=="number" and id>0 then ADMIN_GP_ID=id; break end
                    end
                end
            end
        end
    ...
    while true do
        ...
        for _,p in ipairs(Players:GetPlayers()) do
            ...
            if ADMIN_GP_ID>0 then
                ...
                if ok and owns then has=true end
            end
            if not has then
                for _,attr in ipairs({"Admin","AdminLevel","HasAdmin"}) do
                    if p:GetAttribute(attr) then has=true; break end
                end
            end
        ...
        if adminWarnLbl and adminWarnLbl.Parent then
            if #adminFound>0 then
                adminWarnLbl.Text="!! "..#adminFound.." w/ AP: "..table.concat(adminFound,", ").." !!"
                adminWarnLbl.TextColor3=T.red
            else
                adminWarnLbl.Text="Scanning for AP | "..total.." in server"
                adminWarnLbl.TextColor3=T.dim
            end
        end
    end
end)
...
-- Discord watermark center screen
local discWM=Instance.new("TextLabel",SG); discWM.Size=UDim2.fromOffset(200,22)
discWM.Position=UDim2.new(0.5,-100,0.4,-11); discWM.BackgroundTransparency=1
discWM.Text=DISCORD_LINK; discWM.Font=Enum.Font.GothamBlack; discWM.TextSize=13
discWM.TextColor3=T.a1; discWM.TextTransparency=0.96; discWM.ZIndex=1
TweenService:Create(discWM,TweenInfo.new(2.8,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{TextTransparency=0.52}):Play()

-- Header box
local hdrW=210; local hdrH=100
local hdrF=Instance.new("Frame",SG); hdrF.Size=UDim2.fromOffset(hdrW,hdrH)
hdrF.Position=UDim2.fromOffset(math.floor(vp.X*0.5-hdrW*0.5),4)
hdrF.BackgroundColor3=T.panel; hdrF.BackgroundTransparency=0.14; hdrF.BorderSizePixel=0
co(hdrF,9); rotS(hdrF,T.a1); particles(hdrF,4); hdrF.Active=false
lb(hdrF,{Position=UDim2.fromOffset(0,5),Size=UDim2.new(1,0,0,16),Text="AETHEN HUB  v1.1",
    TextSize=11,Font=Enum.Font.GothamBlack,TextColor3=T.a1,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=4})
lb(hdrF,{Position=UDim2.fromOffset(0,20),Size=UDim2.new(1,0,0,11),Text="r9qbx",
    TextSize=7,Font=Enum.Font.Gotham,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=4})
...
local function hdrBtn(text,pos,w,tc,bg2)
    ...
    b.BackgroundColor3=bg2 or T.surf; b.BackgroundTransparency=0.22; b.Text=text; b.TextColor3=tc or T.a1
    b.Font=Enum.Font.GothamBold; b.TextSize=7; b.BorderSizePixel=0; b.AutoButtonColor=false
    co(b,4); ms(b,1,T.str,0.5); b.ZIndex=5; return b
end
...
local function updGrabHdr()
    ...
    TweenService:Create(grabHdrBtn,TweenInfo.new(0.1),{BackgroundColor3=grabEnabled and Color3.fromRGB(8,22,12) or T.surf}):Play()
end
updGrabHdr()
...
-- Admin warning
local warnF=Instance.new("Frame",SG); warnF.Size=UDim2.fromOffset(300,18)
warnF.Position=UDim2.new(0.5,-150,1,-42); warnF.BackgroundColor3=Color3.fromRGB(10,3,3)
warnF.BackgroundTransparency=0.18; warnF.BorderSizePixel=0; co(warnF,5)
ms(warnF,1,T.red,0.4); warnF.Active=false; warnF.ZIndex=10
...
task.spawn(function()
    while adminWarnLbl and adminWarnLbl.Parent do
        TweenService:Create(adminWarnLbl,TweenInfo.new(0.45,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{TextTransparency=0.72}):Play()
        task.wait(0.45)
        if not adminWarnLbl or not adminWarnLbl.Parent then break end
        TweenService:Create(adminWarnLbl,TweenInfo.new(0.45,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{TextTransparency=0}):Play()
        task.wait(0.45)
    end
end)
...
local tabRow=Instance.new("Frame",FP); tabRow.Size=UDim2.fromOffset(FP_W,20)
tabRow.Position=UDim2.fromOffset(0,22); tabRow.BackgroundColor3=T.surf2; tabRow.BackgroundTransparency=0.1; tabRow.BorderSizePixel=0
local tabMainBtn=Instance.new("TextButton",tabRow); tabMainBtn.Size=UDim2.new(0.5,0,1,0)
tabMainBtn.BackgroundColor3=T.a1; tabMainBtn.Text="MAIN"; tabMainBtn.Font=Enum.Font.GothamBlack
tabMainBtn.TextSize=8; tabMainBtn.TextColor3=T.bg; tabMainBtn.BorderSizePixel=0; tabMainBtn.AutoButtonColor=false; co(tabMainBtn,0)
local tabExtraBtn=Instance.new("TextButton",tabRow); tabExtraBtn.Size=UDim2.new(0.5,0,1,0)
tabExtraBtn.Position=UDim2.new(0.5,0,0,0); tabExtraBtn.BackgroundColor3=T.surf2; tabExtraBtn.Text="EXTRAS"
tabExtraBtn.Font=Enum.Font.GothamBlack; tabExtraBtn.TextSize=8; tabExtraBtn.TextColor3=T.dim
tabExtraBtn.BorderSizePixel=0; tabExtraBtn.AutoButtonColor=false; co(tabExtraBtn,0)
local cont=Instance.new("Frame",FP); cont.Size=UDim2.fromOffset(FP_W,CONT_H)
cont.Position=UDim2.fromOffset(0,43); cont.BackgroundTransparency=1; cont.BorderSizePixel=0; cont.ClipsDescendants=true
...
-- Lag toggle
local lagRow=Instance.new("Frame",pg1); lagRow.Size=UDim2.fromOffset(FP_W-10,21); lagRow.Position=UDim2.fromOffset(5,4)
lagRow.BackgroundColor3=T.surf; lagRow.BackgroundTransparency=0.28; lagRow.BorderSizePixel=0; co(lagRow,6)
local lagRS=ms(lagRow,1,T.str,0.55)
local lagLbl=lb(lagRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Lag On Steal",
    TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local lagTrack=Instance.new("Frame",lagRow); lagTrack.Size=UDim2.fromOffset(25,12)
lagTrack.Position=UDim2.new(1,-29,0.5,-6); lagTrack.BackgroundColor3=T.surf2; lagTrack.BorderSizePixel=0; co(lagTrack,12)
local lagKnob=Instance.new("Frame",lagTrack); lagKnob.Size=UDim2.fromOffset(8,8)
lagKnob.Position=UDim2.new(0,2,0.5,-4); lagKnob.BackgroundColor3=T.a2; lagKnob.BorderSizePixel=0; co(lagKnob,8)
...
-- Potion toggle
local potRow=Instance.new("Frame",pg1); potRow.Size=UDim2.fromOffset(FP_W-10,21); potRow.Position=UDim2.fromOffset(5,28)
potRow.BackgroundColor3=T.surf; potRow.BackgroundTransparency=0.28; potRow.BorderSizePixel=0; co(potRow,6)
local potRS=ms(potRow,1,T.str,0.55)
local potLbl=lb(potRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Giant Potion",
    TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local potTrack=Instance.new("Frame",potRow); potTrack.Size=UDim2.fromOffset(25,12)
potTrack.Position=UDim2.new(1,-29,0.5,-6); potTrack.BackgroundColor3=T.surf2; potTrack.BorderSizePixel=0; co(potTrack,12)
local potK=Instance.new("Frame",potTrack); potK.Size=UDim2.fromOffset(8,8)
potK.Position=UDim2.new(0,2,0.5,-4); potK.BackgroundColor3=T.a2; potK.BorderSizePixel=0; co(potK,8)
...
local sep1=Instance.new("Frame",pg1); sep1.Size=UDim2.new(1,-10,0,1); sep1.Position=UDim2.fromOffset(5,52)
sep1.BackgroundColor3=T.str; sep1.BackgroundTransparency=0.68; sep1.BorderSizePixel=0

-- Flash TP toggle
local togRow=Instance.new("Frame",pg1); togRow.Size=UDim2.fromOffset(FP_W-10,21); togRow.Position=UDim2.fromOffset(5,56)
togRow.BackgroundColor3=T.surf; togRow.BackgroundTransparency=0.28; togRow.BorderSizePixel=0; co(togRow,6)
local togRS=ms(togRow,1,T.str,0.55)
local togLbl=lb(togRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Flash TP",
    TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local pill=Instance.new("Frame",togRow); pill.Size=UDim2.fromOffset(25,12)
pill.Position=UDim2.new(1,-29,0.5,-6); pill.BackgroundColor3=T.surf2; pill.BorderSizePixel=0; co(pill,12)
local knob=Instance.new("Frame",pill); knob.Size=UDim2.fromOffset(8,8)
knob.Position=UDim2.new(0,2,0.5,-4); knob.BackgroundColor3=T.a2; knob.BorderSizePixel=0; co(knob,8)
...
local slBg=Instance.new("Frame",pg1); slBg.Size=UDim2.fromOffset(FP_W-10,5); slBg.Position=UDim2.fromOffset(5,95)
slBg.BackgroundColor3=T.surf; slBg.BorderSizePixel=0; co(slBg,3)
...
local slK=Instance.new("Frame",slBg); slK.Size=UDim2.fromOffset(10,10); slK.AnchorPoint=Vector2.new(0.5,0.5)
slK.Position=UDim2.new(sliderValue,0,0.5,0); slK.BackgroundColor3=T.white; slK.BorderSizePixel=0; co(slK,5)
local slOn=false
local function setTr(pct)
    pct=math.clamp(pct,0,1); sliderValue=pct; Cfg.triggerChance=math.floor(pct*100)
    slFill.Size=UDim2.new(pct,0,1,0); slK.Position=UDim2.new(pct,0,0.5,0)
    trigL.Text="Trigger: "..math.floor(pct*100).."%"
end
slBg.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then
        slOn=true; setTr((i.Position.X-slBg.AbsolutePosition.X)/slBg.AbsoluteSize.X)
    end
end)
...
camBtn.TextSize=9; camBtn.TextColor3=T.a1
...
ssB.MouseButton1Click:Connect(function()
    stealSpdOn=not stealSpdOn; speedEnabled=stealSpdOn; ssSet(stealSpdOn); Cfg.stealSpdOn=stealSpdOn; save()
end)
...
local sep3=Instance.new("Frame",pg2); sep3.Size=UDim2.new(1,-10,0,1); sep3.Position=UDim2.fromOffset(5,48)
sep3.BackgroundColor3=T.str; sep3.BackgroundTransparency=0.68; sep3.BorderSizePixel=0
local brainESPB,brainESPSet=pillR(pg2,"Brainrot ESP",52,T.a1)
...
local friendESPB,friendESPSet=pillR(pg2,"Friends ESP",75,T.grn)
...
local btB,btSet=pillR(pg2,"Brainrot Trans",98,T.a2)
btB.MouseButton1Click:Connect(function()
    if brainTransOn then disableBrainTrans(); btSet(false) else enableBrainTrans(); btSet(true) end
end)
local xrB,xrSet=pillR(pg2,"Base Xray",121,T.red)
...
local apC=mkF(APan,21,AP_CH,apMin); local aY=4
local function apB(text,tc,bg2)
    local b=btn(apC,text,UDim2.fromOffset(AP_W2-10,21),UDim2.fromOffset(5,aY),bg2)
    b.TextColor3=tc or T.a1
    if tc then
        local s2=b:FindFirstChildOfClass("UIStroke")
        if s2 then s2.Color=tc; s2.Transparency=0.3 end
    end
    aY=aY+25; return b
end
local rejB=apB("Rejoin",T.a1)
...
local kickB=apB("Kick Self",T.red,Color3.fromRGB(30,6,6))
kickB.MouseButton1Click:Connect(function() kickB.Text="..."; lp:Kick(DISCORD_LINK) end)
local rstAB=apB("Instant Reset",T.red,Color3.fromRGB(30,6,6))
...
local pScroll=Instance.new("ScrollingFrame",ADMpan); pScroll.Size=UDim2.fromOffset(ADM_W-8,0)
pScroll.Position=UDim2.fromOffset(4,22); pScroll.BackgroundColor3=T.bg; pScroll.BackgroundTransparency=0.65
pScroll.BorderSizePixel=0; pScroll.ScrollBarThickness=2; pScroll.ScrollBarImageColor3=T.a1
pScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; pScroll.CanvasSize=UDim2.new(0,0,0,0); co(pScroll,4)
...
local function refreshPlayers()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        card.BackgroundColor3=T.surf; card.BackgroundTransparency=0.26; card.BorderSizePixel=0; co(card,5)
        local cs=ms(card,1,T.str,0.55)
        local cz=Instance.new("TextButton",card); cz.Size=UDim2.fromOffset(CSTART,30)
        cz.BackgroundTransparency=1; cz.Text=""; cz.AutoButtonColor=false; cz.Parent=card
        local ava=Instance.new("ImageLabel",card); ava.Size=UDim2.fromOffset(18,18)
        ava.Position=UDim2.new(0,3,0.5,-9); ava.BackgroundColor3=T.surf2; ava.BorderSizePixel=0; co(ava,9)
        task.spawn(function()
        ...
        lb(card,{Position=UDim2.new(0,23,0,3),Size=UDim2.fromOffset(CSTART-26,13),Text=p.DisplayName,
            TextSize=9,Font=Enum.Font.GothamBold,TextColor3=T.a1,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd})
        lb(card,{Position=UDim2.new(0,23,0,16),Size=UDim2.fromOffset(CSTART-26,10),Text="@"..p.Name,
            TextSize=7,Font=Enum.Font.Gotham,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd})
        cmdBtnRefs[p.UserId]={}
        for i,def in ipairs(RCMDS) do
            local bx=CSTART+3+(i-1)*(IW2+IG2)
            local ib=Instance.new("TextButton",card); ib.Size=UDim2.fromOffset(IW2,IH2)
            ib.Position=UDim2.new(0,bx,0.5,-IH2/2); ib.BackgroundColor3=T.surf2; ib.BackgroundTransparency=0.22
            ib.Text=def.icon; ib.TextSize=11; ib.TextColor3=T.a1; ib.Font=Enum.Font.GothamBold
            ib.BorderSizePixel=0; ib.AutoButtonColor=false; ib.Parent=card; co(ib,4); ms(ib,1,T.str,0.52)
...
local asB,asSet=pillR(bpC,"Anti Steal",pyy,T.a1); pyy=pyy+23
asB.MouseButton1Click:Connect(function() antiSteal=not antiSteal; asSet(antiSteal) end)
local akB,akSet=pillR(bpC,"Auto Kick",pyy,T.red); pyy=pyy+23
akB.MouseButton1Click:Connect(function() autoKickBase=not autoKickBase; akSet(autoKickBase) end)
local aiOn2=false; local aiConn2=nil; local carpetSpam2={}
local CARPET_CHECK={["Flying Carpet"]=1,["Witch's Broom"]=1,["Santa's Sleigh"]=1,["Cupid's Wings"]=1}
local aiB2,aiSet2=pillR(bpC,"Anti Intruder",pyy,T.a2); pyy=pyy+23
aiB2.MouseButton1Click:Connect(function()
    aiOn2=not aiOn2; aiSet2(aiOn2)
    if aiOn2 and not aiConn2 then
        aiConn2=RunService.Heartbeat:Connect(function()
            if not aiOn2 or not stealHitbox or not ADM_REMOTE then return end
            local cf=stealHitbox.CFrame; local sz=stealHitbox.Size
            local hx=sz.X*0.5; local hz=sz.Z*0.5
            for _,p in ipairs(Players:GetPlayers()) do
                if p~=lp and p.Character then
                    local hrp=p.Character:FindFirstChild("HumanoidRootPart")
                    if hrp then
                        ...
                        for _,item in ipairs(p.Character:GetChildren()) do
                            if CARPET_CHECK[item.Name] then hasCarpet=true; break end
                        end
                        if hasCarpet then
                            local rel=cf:PointToObjectSpace(hrp.Position)
                            if math.abs(rel.X)<=hx and math.abs(rel.Z)<=hz then
                                local uid=p.UserId
                                if not carpetSpam2[uid] then
                                    carpetSpam2[uid]=true
                                    pcall(function() ADM_REMOTE:InvokeServer(ADM_UUID,p,"balloon") end)
                                    rowCdEnd["balloon"]=tick()+(ROW_CD["balloon"] or 30)
                                    if autoKickBase then task.delay(0.05,function() lp:Kick("SAVED BY AETHEN") end) end
                                    task.delay(5,function() carpetSpam2[uid]=nil end)
                                end
                            end
                        end
                    end
    ...
    elseif not aiOn2 and aiConn2 then
        aiConn2:Disconnect(); aiConn2=nil
    end
end)
local arB,arSet=pillR(bpC,"Anti Ragdoll",pyy,T.grn)
arSet(true)
...
rsPnlBtn.MouseButton1Click:Connect(function()
    local popup=Instance.new("Frame",SG); popup.Size=UDim2.fromOffset(188,62)
    popup.Position=UDim2.new(0.5,-94,0.38,-31); popup.BackgroundColor3=T.panel
    popup.BackgroundTransparency=0.08; popup.BorderSizePixel=0; co(popup,10); ms(popup,1.5,T.a1,0.18)
    lb(popup,{Size=UDim2.new(1,0,0,26),Position=UDim2.fromOffset(0,4),Text="Reset panels?",TextSize=9,Font=Enum.Font.GothamBold,TextColor3=T.a1})
