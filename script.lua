...
local Cfg={triggerChance=91,flashOn=false,grabOn=true,potionOn=false,stealSpdOn=false,stealSpdVal=30,pos_FP={ox=0,oy=5},pos_AP={ox=0,oy=0},pos_BP={ox=0,oy=0},pos_LP={ox=6,oy=48}}
pcall(function()
...
for _,v in ipairs(CoreGui:GetChildren()) do
    if v:IsA("ScreenGui") and (v.Name:sub(1,5)=="Aeth_" or v.Name:sub(1,4)=="PhFV") then v:Destroy() end
end
local SG=Instance.new("ScreenGui"); SG.Name="Aeth_09"; SG.ResetOnSpawn=false
...
-- â”€â”€â”€ TIMER ESP + PLOT DETECTION (from Phantom V5) â”€â”€â”€
        ...
        for _,plot in ipairs(plots:GetChildren()) do
            ...
            if tl2 and mp then
                ...
                if not e or not e.bb.Parent then
                    ...
                    local tl=Instance.new("TextLabel",bgT); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1; tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=ECOL_Y
...
local function mkESP(plr)
    ...
    lb(bgE,{Size=UDim2.new(1,0,0.58,0),BackgroundTransparency=1,Text=plr.DisplayName,Font=Enum.Font.GothamBold,TextSize=9,TextColor3=T.a1})
...
local function addMineESP(child)
    ...
    local tl=Instance.new("TextLabel",bgM); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1; tl.Text="MINE: "..child.Name; tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=T.red
...
-- â”€â”€â”€ BASE PROTECTOR (exact from Phantom V5) â”€â”€â”€
...
local function punish(p)
    ...
    if autoKick then task.delay(0.05,function() lp:Kick("SAVED BY AETHEN") end) end
end
...
-- â”€â”€â”€ EXT STEAL CALLBACKS (exact from Phantom V5) â”€â”€â”€
local StealCBCache={}
...
-- â”€â”€â”€ IRISH RESET (exact from Phantom V5) â”€â”€â”€
...
-- â”€â”€â”€ FLASH TP (exact from Phantom V5) â”€â”€â”€
...
-- â”€â”€â”€ CAM ALIGN (exact from Phantom V5) â”€â”€â”€
...
-- â”€â”€â”€ ANTI-RAGDOLL (exact from Phantom V5) â”€â”€â”€
local arConn=nil
...
-- â”€â”€â”€ ANTI-SENTRY (exact from Phantom V5) â”€â”€â”€
local DETECTION_DISTANCE=200; local sentryFirstSeen={}; local sentryTarget=nil
...
-- â”€â”€â”€ ANTI-BEE (exact from Phantom V5) â”€â”€â”€
local FOV_LOCK=70
...
-- â”€â”€â”€ SPEED (from Phantom V5) â”€â”€â”€
...
local function applySpeed()
    ...
    if dir.Magnitude>0 then root.Velocity=Vector3.new(dir.X*stealingSpeed,root.Velocity.Y,dir.Z*stealingSpeed) end
end

-- â”€â”€â”€ GRAB (from Phantom V5) â”€â”€â”€
    ...
    while true do
        if grabEnabled then
            ...
            if hum and hum.Health>0 then
                ...
                if nearest then
                    task.wait(0.07); buildStealCBs(nearest)
...
-- â”€â”€â”€ CMD TIMERS + HEARTBEAT â”€â”€â”€
...
wmL.BackgroundTransparency=1; wmL.Text="Aethen Hub v0.9"; wmL.Font=Enum.Font.GothamBlack; wmL.TextSize=13; wmL.TextColor3=T.a1; wmL.TextTransparency=0.96
...
local fpsLb=lb(fpsF,{Size=UDim2.new(1,0,1,0),Text="FPS --  Â·  PING -- ms",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.a1})
task.spawn(function()
    while task.wait(0.7) do
        ...
        fpsLb.Text="FPS "..fps.."  Â·  PING "..ping.."ms"
        fpsLb.TextColor3=fps<40 and T.red or fps<55 and Color3.fromRGB(255,190,40) or T.grn
    end
end)

-- Reset panels btn
local rsPnlBtn=Instance.new("TextButton",SG); rsPnlBtn.Size=UDim2.fromOffset(88,13); rsPnlBtn.Position=UDim2.new(0.5,-44,0,43)
rsPnlBtn.BackgroundColor3=T.panel; rsPnlBtn.BackgroundTransparency=0.42; rsPnlBtn.Text="Reset Panels"; rsPnlBtn.Font=Enum.Font.GothamBold; rsPnlBtn.TextSize=7; rsPnlBtn.TextColor3=T.dim; rsPnlBtn.BorderSizePixel=0; rsPnlBtn.AutoButtonColor=false; co(rsPnlBtn,4); ms(rsPnlBtn,1,T.str,0.55)

-- â”€â”€ PANEL A: Aethen Hub â”€â”€
local LP,_,lpMin=mkP(6,48,LP_W,LP_H,"Aethen Hub v0.9",T.a1,"pos_LP")
...
local kickB=lpB("Kick Self"); kickB.TextColor3=T.red
do local s=kickB:FindFirstChildOfClass("UIStroke"); if s then s.Color=T.red; s.Transparency=0.3 end end
kickB.MouseButton1Click:Connect(function() kickB.Text="..."; lp:Kick("Aethen Hub") end)

local grabB=lpB("Instant Grab: ON"); grabB.TextSize=8
local function updG() grabB.Text=grabEnabled and "Instant Grab: ON" or "Instant Grab: OFF"; TweenService:Create(grabB,TweenInfo.new(0.1),{BackgroundColor3=grabEnabled and Color3.fromRGB(12,22,12) or T.surf}):Play(); grabB.TextColor3=grabEnabled and T.grn or T.white end
...
camB.MouseButton1Click:Connect(function()
    ...
    task.delay(1,function() camB.Text="ALLIGN CAMERA"; camBusy=false end)
end)

-- â”€â”€ PANEL B: Flash TP (exact from Phantom V5) â”€â”€
local FP,_,fpMin=mkP(0,5,FP_W,FP_H,"Flash TP",T.a2,"pos_FP")
local fpC=mkF(FP,21,FP_CH,fpMin)

local togRow=Instance.new("Frame"); togRow.Size=UDim2.fromOffset(FP_W-10,21); togRow.Position=UDim2.fromOffset(5,4); togRow.BackgroundColor3=T.surf; togRow.BackgroundTransparency=0.28; togRow.BorderSizePixel=0; togRow.Parent=fpC; co(togRow,6)
...
local potRow=Instance.new("Frame"); potRow.Size=UDim2.fromOffset(FP_W-10,21); potRow.Position=UDim2.fromOffset(5,70); potRow.BackgroundColor3=T.surf; potRow.BackgroundTransparency=0.28; potRow.BorderSizePixel=0; potRow.Parent=fpC; co(potRow,6)
...
apMin.MouseButton1Click:Connect(function()
    apFolded=not apFolded; apMin.Text=apFolded and "+" or "â”€"; pScroll.Visible=not apFolded
    if apFolded then
        TweenService:Create(AP,TweenInfo.new(0.2,Enum.EasingStyle.Quad),{Size=UDim2.fromOffset(AP_W,24)}):Play()
    else
        local ch=math.min(pLayout.AbsoluteContentSize.Y+4,120)
        TweenService:Create(AP,TweenInfo.new(0.2,Enum.EasingStyle.Quad),{Size=UDim2.fromOffset(AP_W,22+ch+3)}):Play()
    end
end)
local pCards={}
...
-- â”€â”€ PANEL D: Base Protector (exact from Phantom V5) â”€â”€
...
local arB,arSet=bpRow("ANTI RAGDOLL",T.grn); arSet(true)
arB.MouseButton1Click:Connect(function()
    if arConn then arConn:Disconnect(); arConn=nil; arSet(false)
    else startAntiRagdoll(); arSet(true) end
end)

-- â”€â”€ PANEL E: Speed â”€â”€
...
ssB.MouseButton1Click:Connect(function()
    stealSpdOn=not stealSpdOn; isStealing=stealSpdOn; speedEnabled=stealSpdOn; ssSet(stealSpdOn); Cfg.stealSpdOn=stealSpdOn; save()
end)
