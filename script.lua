...
pcall(function()
    ...
    -- Sounds parented to SoundService (not workspace) so they don't affect spatial audio
    local sounds={}
    local function ps(id,vol)
        local s=Instance.new("Sound",SS)
        s.SoundId="rbxassetid://"..tostring(id)
        s.Volume=vol s.Looped=true s.RollOffMaxDistance=0
        pcall(function() s:Play() end)
        table.insert(sounds,s) return s
    end
    ps(2865227914,0.7) ps(1372077950,0.45) ps(4877219993,0.35)
    ...
    pcall(function()
        ...
        doneS.Volume=0.45 doneS:Play() game:GetService("Debris"):AddItem(doneS,3)
    end)
...
UIS.InputBegan:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch then touchMoved=false touchStartPos=Vector2.new(i.Position.X,i.Position.Y) end
end)
UIS.InputChanged:Connect(function(i)
    if i.UserInputType==Enum.UserInputType.Touch then
        if(Vector2.new(i.Position.X,i.Position.Y)-touchStartPos).Magnitude>14 then touchMoved=true end
    end
end)
...
RunService.Heartbeat:Connect(function()
    ...
    if stealBarFill then
        if not best then stealBarFill.Size=UDim2.new(0,0,1,0) tw(stealBarFill,{BackgroundColor3=C.LIME},0.1)
            ...
            tw(stealBarFill,{BackgroundColor3=bd<=GRAB_DIST and C.GREEN or C.LIME},0.08)
        end
    end
    if stealBarTxt then
...
-- ====== FAST TP (slowed down more) ======
local WAYPOINTS={
    Vector3.new(-362.2,-7.3,70.9),
    Vector3.new(-360.6,-7.3,22.6),
    Vector3.new(-331.8,-5.1,22.7),
}
local TP_DELAY=0.02 -- slightly slower so each waypoint registers
...
-- ====== FRIEND REQUEST (fixed: debounce + 1sec delay) ======
...
local function fireFriendRequests()
    ...
    friendBusy=true
    task.spawn(function()
        task.wait(1) -- 1 second delay as requested
...
-- Auto loop - only runs when on, with generous gap
task.spawn(function()
    while sg.Parent do
        task.wait(5)
...
local function makePanel(name,w,startPos,accent)
    ...
    frame.BackgroundTransparency=0.22 frame.BorderSizePixel=0 frame.Active=true frame.AutomaticSize=Enum.AutomaticSize.Y
    ...
    local hdr=Instance.new("Frame",frame) hdr.Size=UDim2.new(1,0,0,26) hdr.BackgroundColor3=C.CARD hdr.BackgroundTransparency=0.2 hdr.BorderSizePixel=0 hdr.LayoutOrder=1
    local hdrFix=Instance.new("Frame",hdr) hdrFix.Size=UDim2.new(1,0,0.5,0) hdrFix.Position=UDim2.new(0,0,0.5,0) hdrFix.BackgroundColor3=C.CARD hdrFix.BackgroundTransparency=0.2 hdrFix.BorderSizePixel=0
    ...
    hkBtn.BackgroundColor3=C.CARD2 hkBtn.BackgroundTransparency=0.2 hkBtn.BorderSizePixel=0 hkBtn.Text="?" hkBtn.Font=Enum.Font.GothamBold hkBtn.TextSize=7 hkBtn.TextColor3=C.DIM hkBtn.AutoButtonColor=false
    ...
    xBtn.BackgroundColor3=Color3.fromRGB(32,9,9) xBtn.BackgroundTransparency=0.15 xBtn.BorderSizePixel=0 xBtn.Text="×" xBtn.Font=Enum.Font.GothamBold xBtn.TextSize=13 xBtn.TextColor3=C.CORAL xBtn.AutoButtonColor=false co(xBtn,4)
...
local function makeRow(parent,label,order,accent)
    accent=accent or C.LIME
    local row=Instance.new("Frame",parent) row.Size=UDim2.new(1,0,0,28) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=0.22 row.BorderSizePixel=0 row.LayoutOrder=order co(row,6)
...
local function makeBtn(parent,txt,col,order)
    ...
    btn.BackgroundColor3=Color3.fromRGB(math.floor(r2*0.08),math.floor(g2*0.08),math.floor(b2*0.08))
    btn.BackgroundTransparency=0.1 btn.BorderSizePixel=0 btn.Text=txt btn.Font=Enum.Font.GothamBold btn.TextSize=9 btn.TextColor3=col btn.AutoButtonColor=false btn.LayoutOrder=order or 99
...
local function makeReopener(txt,pos,col,openFn)
    ...
    b.BackgroundColor3=C.CARD2 b.BackgroundTransparency=0.22 b.BorderSizePixel=0 b.Text=txt b.Font=Enum.Font.GothamBlack b.TextSize=7 b.TextColor3=col b.Visible=false b.ZIndex=20
...
grabBar.BackgroundColor3=C.BG grabBar.BackgroundTransparency=0.18 grabBar.BorderSizePixel=0
...
-- ====== FRIEND BAR (y=40, slightly lower) ======
local friendBar=Instance.new("Frame",sg)
friendBar.Size=UDim2.fromOffset(192,34)
friendBar.AnchorPoint=Vector2.new(0.5,0)
friendBar.Position=UDim2.new(0.5,0,0,40) -- moved slightly lower
friendBar.BackgroundColor3=Color3.fromRGB(55,8,8)
friendBar.BackgroundTransparency=0.05
friendBar.BorderSizePixel=0
co(friendBar,9)
local fbStroke=stk(friendBar,C.CORAL,2.2)
makeDrag(friendBar)

local fbLbl=Instance.new("TextLabel",friendBar)
fbLbl.Size=UDim2.new(1,-34,1,0) fbLbl.Position=UDim2.fromOffset(0,0)
...
fbHkBtn.Size=UDim2.fromOffset(28,20) fbHkBtn.Position=UDim2.new(1,-32,0.5,-10)
...
-- Click button, excludes hotkey area
...
-- Debounce for friend bar toggle to prevent flicker
local friendToggleCooldown=false
local function setFriendBar(state)
    if friendToggleCooldown then return end
    friendToggleCooldown=true
    friendOn=state
    if state then
        tw(friendBar,{BackgroundColor3=Color3.fromRGB(8,55,14),BackgroundTransparency=0.05},0.18)
        TweenService:Create(fbStroke,TweenInfo.new(0.01),{Transparency=0.1,Color=C.GREEN}):Play()
        TweenService:Create(fbStroke,TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut,-1,true),{Transparency=0.6,Color=C.LIME}):Play()
    ...
    task.delay(0.5,function() friendToggleCooldown=false end)
end
setFriendBar(false)
...
-- ====== AP PANEL ======
local APFr,APCnt,APX,APHk=makePanel("AP SPAM",148,UDim2.fromOffset(42,82),C.LIME)
APFr.Visible=false
local APReo=makeReopener("AP",UDim2.fromOffset(42,82),C.LIME,function() APFr.Visible=true end)
...
local function buildAPCards()
    ...
    for i,p in ipairs(plist) do
        ...
        local card=Instance.new("Frame",apScr) card.Size=UDim2.new(1,-2,0,28) card.BackgroundColor3=C.CARD2 card.BackgroundTransparency=0.22 card.BorderSizePixel=0 card.LayoutOrder=i co(card,5)
        ...
        safeClick(cb,function()
            ...
            for _,c2 in ipairs(apScr:GetChildren()) do if c2:IsA("Frame") then
                c2.BackgroundColor3=C.CARD2 c2.BackgroundTransparency=0.22
...
-- ====== MINI AP PANEL ======
local MAFr,MACnt,MAX2,MAHk=makePanel("MINI AP",218,UDim2.fromOffset(198,82),C.CYAN)
MAFr.Visible=false
local MAReo=makeReopener("MA",UDim2.fromOffset(198,82),C.CYAN,function() MAFr.Visible=true end)
...
local function buildMiniCards()
    ...
    for i,p in ipairs(Players:GetPlayers()) do
        if p==lp then continue end
        local row=Instance.new("Frame",maScr) row.Size=UDim2.new(1,-2,0,26) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=0.22 row.BorderSizePixel=0 row.LayoutOrder=i co(row,5) stk(row,C.BORD,1)
        ...
        for _,cmd in ipairs(MINI_CMDS) do
            local btn=Instance.new("TextButton",row) btn.Size=UDim2.fromOffset(30,20) btn.Position=UDim2.new(0,xOff,0.5,-10) btn.BackgroundColor3=cmd.col btn.BackgroundTransparency=0.15 btn.BorderSizePixel=0 btn.Text=cmd.e btn.Font=Enum.Font.GothamBold btn.TextSize=11 btn.AutoButtonColor=false co(btn,4)
...
-- ====== BLOCK PANEL ======
local BKFr,BKCnt,BKX,BKHk=makePanel("BLOCK",148,UDim2.fromOffset(42,190),C.CORAL)
BKFr.Visible=false
local BKReo=makeReopener("BL",UDim2.fromOffset(42,190),C.CORAL,function() BKFr.Visible=true end)
...
local function buildBlockList()
    ...
    for i,p in ipairs(Players:GetPlayers()) do
        if p==lp then continue end
        local row=Instance.new("Frame",bkScr) row.Size=UDim2.new(1,-2,0,24) row.BackgroundColor3=C.CARD2 row.BackgroundTransparency=0.22 row.BorderSizePixel=0 row.LayoutOrder=i co(row,5) stk(row,C.BORD,1)
...
local rsStatRow=Instance.new("Frame",RSCnt) rsStatRow.Size=UDim2.new(1,0,0,18) rsStatRow.BackgroundColor3=C.CARD2 rsStatRow.BackgroundTransparency=0.22 rsStatRow.BorderSizePixel=0 rsStatRow.LayoutOrder=3 co(rsStatRow,5) stk(rsStatRow,C.BORD,1)
...
-- ====== TP PANEL ======
local TPFr,TPCnt,TPX,TPHk=makePanel("FAST TP",144,UDim2.new(1,-152,0,82),C.CYAN)
local TPReo=makeReopener("TP",UDim2.new(1,-44,0,82),C.CYAN,function() TPFr.Visible=true end)
...
local espRow,espRS,espLbl2=makeRow(TPCnt,"Player ESP",6,C.ELEC)
...
local function sideBtn(label,col,cb2)
    local f=Instance.new("Frame",sideF) f.Size=UDim2.fromOffset(30,30) f.BackgroundColor3=C.BG f.BackgroundTransparency=0.18 f.BorderSizePixel=0 co(f,8) stk(f,col,1.2)
...
if isMobile then
    local function mobBtn(txt,yOff,col,cb2)
        local f=Instance.new("Frame",sg) f.Size=UDim2.fromOffset(44,44) f.Position=UDim2.new(1,-50,0.5,yOff) f.BackgroundColor3=C.BG f.BackgroundTransparency=0.22 f.BorderSizePixel=0 co(f,10) stk(f,col,1.2)
...
-- ====== PLAYER LIST MANAGEMENT (fixed) ======
-- All lists update when panels are visible OR when toggled open
local function doRefresh()
    ...
    if playerESPOn then
        for _,p in ipairs(Players:GetPlayers()) do
            if p~=lp and p.Character and not p.Character:FindFirstChild("PhESP_Box") then mkESP(p) end
        end
    end
end

-- Hook every player + their characters robustly
local function hookPlayer(p)
    if p==lp then return end
    -- Always add CharacterAdded hook for ESP + list refresh
    ...
    if p.Character then
        if playerESPOn then task.spawn(function() mkESP(p) end) end
    end
end
...
Players.PlayerAdded:Connect(function(p)
    hookPlayer(p)
    task.wait(0.5) -- small wait so character can load
    task.spawn(doRefresh)
end)
...
-- Periodic refresh every 8 seconds as safety net
task.spawn(function()
    while sg.Parent do
        task.wait(8)
        task.spawn(doRefresh)
...
UIS.InputBegan:Connect(function(inp,gpe)
    ...
    if waitingHotkey then
        if inp.UserInputType==Enum.UserInputType.Keyboard and inp.KeyCode~=Enum.KeyCode.Unknown then
            hotkeys[waitingHotkey]=inp.KeyCode waitingHotkey=nil
        end return
    end
