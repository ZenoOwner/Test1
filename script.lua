--  Aethen Hub v1.1 | r9qbx on discord
...
local Lighting=game:GetService("Lighting"); local MPS=game:GetService("MarketplaceService")
local lp=Players.LocalPlayer; local cam=workspace.CurrentCamera

local CFG="AethonV11.json"
local Cfg={triggerChance=91,flashOn=false,grabOn=false,potionOn=false,
...
for _,v in ipairs(CoreGui:GetChildren()) do
    if v:IsA("ScreenGui") and (v.Name:sub(1,4)=="Aeth" or v.Name:sub(1,4)=="PhFV" or v.Name:sub(1,3)=="Von") then v:Destroy() end
end
local SG=Instance.new("ScreenGui"); SG.Name="Aeth11_"..tostring(math.random(1000,9999))
SG.ResetOnSpawn=false; SG.ZIndexBehavior=Enum.ZIndexBehavior.Global; SG.IgnoreGuiInset=true; SG.Parent=CoreGui

local T={
    bg=Color3.fromRGB(5,5,9),panel=Color3.fromRGB(7,7,13),surf=Color3.fromRGB(12,12,20),surf2=Color3.fromRGB(18,18,30),
    a1=Color3.fromRGB(225,225,240),a2=Color3.fromRGB(170,170,195),white=Color3.new(1,1,1),
    dim=Color3.fromRGB(100,100,122),str=Color3.fromRGB(150,150,178),red=Color3.fromRGB(255,65,65),
    grn=Color3.fromRGB(52,218,88),yel=Color3.fromRGB(255,200,40),
}
...
local function rotS(f,col)
    local s=ms(f,1.3,col,0.22); local g=Instance.new("UIGradient",s)
    g.Color=ColorSequence.new{ColorSequenceKeypoint.new(0,Color3.new(1,1,1)),ColorSequenceKeypoint.new(0.25,Color3.new(0,0,0)),ColorSequenceKeypoint.new(0.5,col or T.a1),ColorSequenceKeypoint.new(0.75,Color3.new(0,0,0)),ColorSequenceKeypoint.new(1,Color3.new(1,1,1))}
    task.spawn(function() while s and s.Parent do g.Rotation=(g.Rotation+0.9)%360; task.wait(0.016) end end)
end
local function particles(parent,count)
    for i=1,count or 4 do
        local p=Instance.new("Frame",parent); p.BackgroundColor3=Color3.fromRGB(200,210,255); p.BackgroundTransparency=0.48; p.BorderSizePixel=0; p.ZIndex=0
        local sz=math.random(2,4); p.Size=UDim2.fromOffset(sz,sz); local sx=math.random(); p.Position=UDim2.new(sx,0,1.05,0); co(p,5)
        task.spawn(function()
            while p and p.Parent do
                ...
                if p and p.Parent then p.BackgroundTransparency=0.48; sx=math.random(); p.Position=UDim2.new(sx,0,1.05,0) end
            end
        end)
    end
end
...
local function mkP(defX,defY,w,h,title,ac,posKey)
    ...
    f.BackgroundColor3=T.panel; f.BackgroundTransparency=0.14; f.BorderSizePixel=0; f.ClipsDescendants=true; f.Parent=SG; co(f,9)
    rotS(f,ac or T.a1); particles(f,4)
    local hdr=Instance.new("Frame",f); hdr.Size=UDim2.new(1,0,0,21); hdr.BackgroundColor3=T.surf; hdr.BackgroundTransparency=0.18; hdr.BorderSizePixel=0; co(hdr,9)
...
local function pillR(parent,label,yPos,onCol)
    local row=Instance.new("Frame"); row.Size=UDim2.new(1,-8,0,20); row.Position=UDim2.new(0,4,0,yPos); row.BackgroundColor3=T.surf; row.BackgroundTransparency=0.28; row.BorderSizePixel=0; row.Parent=parent; co(row,6)
    ...
    local track=Instance.new("Frame",row); track.Size=UDim2.fromOffset(25,12); track.Position=UDim2.new(1,-29,0.5,-6); track.BackgroundColor3=T.surf2; track.BorderSizePixel=0; co(track,12)
    local knob=Instance.new("Frame",track); knob.Size=UDim2.fromOffset(8,8); knob.Position=UDim2.new(0,2,0.5,-4); knob.BackgroundColor3=T.a2; knob.BorderSizePixel=0; co(knob,8)
...
local function numRow(parent,label,val,mn,mx,yPos,w,onChange)
    ...
    mB.MouseButton1Click:Connect(function() setV(cur-1) end); pB.MouseButton1Click:Connect(function() setV(cur+1) end)
...
if not _G._AethBGv11 then _G._AethBGv11=true

    pcall(function() Lighting.GlobalShadows=false; Lighting.FogEnd=1e9
        for _,v in pairs(workspace:GetDescendants()) do if v:IsA("ParticleEmitter") or v:IsA("PointLight") then pcall(function() v.Enabled=false end) end end end)
    ...
    -- Anti-ragdoll (from Phantom Hub V5)
    ...
    -- Anti-bee: FIXED - do NOT scan for "bee" in tool names (breaks Beehive tool)
    -- Only destroy clear bee-sting effect values/scripts, not tools
    RunService:BindToRenderStep("AethAntiBee",Enum.RenderPriority.Input.Value+1,function()
        ...
        for _,obj in ipairs(root:GetChildren()) do if obj:IsA("BodyGyro") or obj:IsA("BodyAngularVelocity") then pcall(function() obj:Destroy() end) end end
        -- Only remove non-tool objects with these names (won't delete Beehive tool)
        for _,obj in ipairs(char:GetDescendants()) do
            if obj:IsA("Tool") or obj:IsA("Backpack") then continue end
            local nl=obj.Name:lower()
            if nl=="invertmovement" or nl=="reverseinput" or nl=="confuse" or (nl=="beeeffect" and not obj:IsA("Tool")) then
                pcall(function() obj:Destroy() end)
            end
        end
        -- Only correct velocity if VERY clearly inverted (dot < -0.65)
    ...
    for _,v in pairs(Lighting:GetDescendants()) do if beeBL[v.ClassName] then pcall(function() v:Destroy() end) end end
    Lighting.DescendantAdded:Connect(function(obj) task.wait(); if beeBL[obj.ClassName] then pcall(function() obj:Destroy() end) end end)

    -- Anti-sentry v2: waits for deployment, skips during steal, only moves primary part
        ...
        for _,obj in pairs(workspace:GetChildren()) do
            if (obj.Name:find("Sentry") or obj.Name:find("Beehive") or obj.Name:find("Turret")) and not obj.Name:lower():find("bullet") then
                ...
                if part and (rp-part.Position).Magnitude<=220 then
                    ...
                    -- Wait 3s for deployment before attacking
    ...
    local function moveSentry(obj)
        ...
        -- Only move primary part (not all descendants - was causing lag)
...
task.spawn(function() while true do task.wait(0.9)
    if not myPlotRef then
        ...
        for _,p in ipairs(plots:GetChildren()) do
            ...
            local tl=sign:FindFirstChild("TextLabel",true); if tl then local t=tl.Text:lower(); if t:find(lp.Name:lower(),1,true) or t:find(lp.DisplayName:lower(),1,true) then myPlotRef=p; stealHitbox=p:FindFirstChild("StealHitbox",true); break end end
        end
    end
end end)
...
local function isMyPlot(plot)
    ...
    local yb=sign:FindFirstChild("YourBase"); return yb and yb:IsA("BillboardGui") and yb.Enabled
end
...
local function showAlert()
    ...
    local af=Instance.new("Frame",ag); af.Size=UDim2.fromOffset(182,26); af.Position=UDim2.new(0.5,-91,0,-34); af.BackgroundColor3=T.panel; af.BackgroundTransparency=0.08; af.BorderSizePixel=0; co(af,7); ms(af,1.5,T.red,0.08)
    ...
    for _,plot in ipairs(plots:GetChildren()) do
        ...
        if tl2 and mp then
            ...
            if not e or not e.bb.Parent then
                ...
                local bgT=Instance.new("Frame",bb); bgT.Size=UDim2.new(1,0,1,0); bgT.BackgroundColor3=T.bg; bgT.BackgroundTransparency=0.18; bgT.BorderSizePixel=0; co(bgT,4)
                local stk=Instance.new("UIStroke",bgT); stk.Color=T.yel; stk.Thickness=0.8; stk.Transparency=0.28; stk.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
                local tl=Instance.new("TextLabel",bgT); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1; tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=T.yel; tl.TextStrokeTransparency=0.45
                timerESPs[plot.Name]={bb=bb,lbl=tl,stk=stk}; e=timerESPs[plot.Name]
            end
            e.lbl.Text=tl2.Text
            local m,s2=tl2.Text:match("(%d+):(%d+)")
            if m and s2 then
                local tot=tonumber(m)*60+tonumber(s2); local col=tot<=30 and T.red or tot<=60 and T.yel or T.grn
...
local function mkESP(plr)
    ...
    local bgE=Instance.new("Frame",bb); bgE.Size=UDim2.new(1,0,1,0); bgE.BackgroundColor3=T.bg; bgE.BackgroundTransparency=0.22; bgE.BorderSizePixel=0; co(bgE,5)
    local se=Instance.new("UIStroke",bgE); se.Thickness=1; se.Color=T.a1; se.Transparency=0.28; se.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    lb(bgE,{Size=UDim2.new(1,0,0.55,0),BackgroundTransparency=1,Text=plr.DisplayName,Font=Enum.Font.GothamBold,TextSize=9,TextColor3=T.a1,TextStrokeTransparency=0.45})
    lb(bgE,{Size=UDim2.new(1,0,0.45,0),Position=UDim2.new(0,0,0.55,0),BackgroundTransparency=1,Text="@"..plr.Name,Font=Enum.Font.Gotham,TextSize=7,TextColor3=T.dim})
end
...
local function addMineESP(child)
    ...
    local part=child:IsA("BasePart") and child or (child:IsA("Model") and (child.PrimaryPart or child:FindFirstChildWhichIsA("BasePart"))); if not part then return end
    ...
    local bgM=Instance.new("Frame",bb); bgM.Size=UDim2.new(1,0,1,0); bgM.BackgroundColor3=Color3.fromRGB(22,4,4); bgM.BackgroundTransparency=0.18; bgM.BorderSizePixel=0; co(bgM,4)
    local sm=Instance.new("UIStroke",bgM); sm.Color=T.red; sm.Thickness=1; sm.Transparency=0.18; sm.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    local tl=Instance.new("TextLabel",bgM); tl.Size=UDim2.new(1,0,1,0); tl.BackgroundTransparency=1; tl.Text="MINE: "..child.Name; tl.Font=Enum.Font.GothamBold; tl.TextSize=9; tl.TextColor3=T.red; tl.TextStrokeTransparency=0.4
    mineESPs[child]={bb=bb}
    child.AncestryChanged:Connect(function() if not child.Parent then pcall(function() bb:Destroy() end); mineESPs[child]=nil end end)
end
...
local function addBrainrotESP(model,plot)
    ...
    local part=model:IsA("Model") and (model.PrimaryPart or model:FindFirstChildWhichIsA("BasePart")) or (model:IsA("BasePart") and model); if not part then return end
    local owner=""; if plot then local sign=plot:FindFirstChild("PlotSign"); if sign then local tl=sign:FindFirstChild("TextLabel",true); if tl then owner=tl.Text end end end
    local bb=Instance.new("BillboardGui"); bb.Name="AethBrainrotESP"; bb.Size=UDim2.fromOffset(112,26); bb.StudsOffset=Vector3.new(0,5,0); bb.AlwaysOnTop=true; bb.Adornee=part; bb.Parent=workspace
    local bg=Instance.new("Frame",bb); bg.Size=UDim2.new(1,0,1,0); bg.BackgroundColor3=T.bg; bg.BackgroundTransparency=0.22; bg.BorderSizePixel=0; co(bg,5)
    local bs=Instance.new("UIStroke",bg); bs.Thickness=1; bs.Color=T.a2; bs.Transparency=0.22; bs.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    lb(bg,{Size=UDim2.new(1,0,0,14),Position=UDim2.fromOffset(0,2),BackgroundTransparency=1,Text=model.Name,Font=Enum.Font.GothamBold,TextSize=9,TextColor3=T.a1,TextTruncate=Enum.TextTruncate.AtEnd})
    if owner~="" then lb(bg,{Size=UDim2.new(1,-4,0,10),Position=UDim2.fromOffset(2,14),BackgroundTransparency=1,Text=owner,Font=Enum.Font.Gotham,TextSize=7,TextColor3=T.dim,TextTruncate=Enum.TextTruncate.AtEnd}) end
...
local function scanSpawn(spawn,plot) for _,child in ipairs(spawn:GetChildren()) do addBrainrotESP(child,plot) end; local c=spawn.ChildAdded:Connect(function(child) if brainrotESPEnabled then task.defer(function() addBrainrotESP(child,plot) end) end end); table.insert(brainrotConns,c) end
local function startBrainrotESP()
    ...
    for _,plot in ipairs(plots:GetChildren()) do
        ...
        local c=pods.ChildAdded:Connect(function(pod) if brainrotESPEnabled then local base=pod:FindFirstChild("Base"); local spawn=base and base:FindFirstChild("Spawn"); if spawn then scanSpawn(spawn,plot) end end end); table.insert(brainrotConns,c)
    end; local c2=plots.ChildAdded:Connect(function(plot) if brainrotESPEnabled then task.wait(0.5); local pods=plot:FindFirstChild("AnimalPodiums"); if not pods then return end; for _,pod in ipairs(pods:GetChildren()) do local base=pod:FindFirstChild("Base"); local spawn=base and base:FindFirstChild("Spawn"); if spawn then scanSpawn(spawn,plot) end end end end); table.insert(brainrotConns,c2)
end
...
local function buildFriendBB(main,isOpen)
    local bb=Instance.new("BillboardGui"); bb.Adornee=main; bb.Size=UDim2.fromOffset(52,18); bb.StudsOffset=Vector3.new(0,5,0); bb.AlwaysOnTop=true; bb.Parent=main
    local bg=Instance.new("Frame",bb); bg.Size=UDim2.new(1,0,1,0); bg.BackgroundColor3=T.bg; bg.BackgroundTransparency=0.22; bg.BorderSizePixel=0; co(bg,5)
    local bs=Instance.new("UIStroke",bg); bs.Thickness=1; bs.Color=isOpen and T.grn or T.red; bs.Transparency=0.22; bs.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
    lb(bg,{Size=UDim2.new(1,0,1,0),BackgroundTransparency=1,Text=isOpen and "OPEN" or "CLOSED",TextColor3=isOpen and T.grn or T.red,Font=Enum.Font.GothamBold,TextSize=9})
    return bb
end
local function addFriendESP(plot)
    if not plot:IsA("Model") then return end; local fp=plot:FindFirstChild("FriendPanel"); if not fp then return end
    local main=fp:FindFirstChild("Main"); if not main then return end; local prompt=main:FindFirstChildOfClass("ProximityPrompt"); if not prompt then return end
    ...
    local c=prompt:GetPropertyChangedSignal("ObjectText"):Connect(function()
        ...
        if friendESPCache[plot] then pcall(function() friendESPCache[plot]:Destroy() end); friendESPCache[plot]=nil end
        friendESPCache[plot]=buildFriendBB(main,prompt.ObjectText=="Disallow Friends")
    end); table.insert(friendConns,c)
    plot.AncestryChanged:Connect(function() if not plot.Parent then if friendESPCache[plot] then pcall(function() friendESPCache[plot]:Destroy() end) end; friendESPCache[plot]=nil end end)
end
local function startFriendESP()
    ...
    local c=plots.ChildAdded:Connect(function(p) if friendESPEnabled then task.wait(0.5); addFriendESP(p) end end); table.insert(friendConns,c)
end
...
local function enableXray() if xrayOn then return end; xrayOn=true; for _,obj in ipairs(workspace:GetDescendants()) do if obj:IsA("BasePart") then local n=obj.Name:lower(); if n:find("floor") or n:find("wall") or n:find("base") or n:find("plot") or n:find("ceil") then if not xrayOrig[obj] then xrayOrig[obj]=obj.Transparency end; obj.Transparency=0.72 end end end end
local function disableXray() if not xrayOn then return end; xrayOn=false; for obj,t in pairs(xrayOrig) do pcall(function() if obj and obj.Parent then obj.Transparency=t end end) end; xrayOrig={} end
...
task.spawn(function()
    ...
    pcall(function()
        ...
        local a=n2i["RF/a0e78691-cb9b-4efc-ac08-9c06fea70059"]; if a and ch[a+1] then ADM_REMOTE=ch[a+1] end
    end)
end)
...
local function fireClick(tgt) if not ADM_REMOTE then return end; for _,cmd in ipairs(CLICK_CMDS) do aFire(tgt,cmd); if ROW_CD[cmd] then rowCdEnd[cmd]=tick()+ROW_CD[cmd] end end end
...
local function activatePotion()
    ...
    task.spawn(function() pcall(function()
        local char=lp.Character; if not char then return end
        local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end
        local bp=lp:FindFirstChild("Backpack"); local tool=char:FindFirstChild("Giant Potion") or (bp and bp:FindFirstChild("Giant Potion")); if not tool then return end
        hum:EquipTool(tool); task.wait(0.08); tool:Activate()
    end) end)
end

-- Irish Hub instant reset (Phantom Hub V5)
...
local function equipFlash()
    pcall(function()
        local char=lp.Character; if not char then return end
        local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end
        for _,n in ipairs(FLASH_NAMES) do local t=char:FindFirstChild(n) or (lp.Backpack and lp.Backpack:FindFirstChild(n)); if t then if t.Parent~=char then hum:EquipTool(t) end; return end end
    end)
end
...
task.spawn(function() while true do task.wait(0.5)
    pcall(function()
        ...
        if fa then for _,v in ipairs(fa:GetDescendants()) do if v:IsA("ParticleEmitter") or v:IsA("Beam") or v:IsA("Trail") then pcall(function() v.Enabled=false end) end; if v:IsA("Sound") then pcall(function() v.Volume=0 end) end end end
    end)
    pcall(function() local char=lp.Character; if not char then return end
        for _,n in ipairs(FLASH_NAMES) do local t=char:FindFirstChild(n); if t then for _,v in ipairs(t:GetDescendants()) do if v:IsA("ParticleEmitter") or v:IsA("Beam") or v:IsA("Trail") then pcall(function() v.Enabled=false end) end; if v:IsA("Sound") then pcall(function() v.Volume=0 end) end end end end
    end)
end end)

-- Strong lag (busy wait 50ms)
local function lagSelf() local deadline=tick()+0.05; local x=0; while tick()<deadline do for i=1,500 do x=x+math.sqrt(i)*0.001 end end; return x end

-- Cam align (EXACT from Phantom Hub V5)
...
local function buildLook(baseLook) local h2=flattenV(baseLook); local fs=math.sqrt(math.max(0,1-desiredY^2)); return Vector3.new(h2.X*fs,desiredY,h2.Z*fs) end
...
-- EXT steal callbacks (EXACT from Phantom Hub V5)
local StealCBCache={}
...
-- FLASH TP (EXACT from Phantom Hub V5, + optional lag + RF bypass)
local flashEnabled=Cfg.flashOn; local sliderValue=Cfg.triggerChance/100; local activePrompts={}; local lagOnSteal=false
PPS.PromptButtonHoldBegan:Connect(function(prompt)
    ...
    conn=RunService.Heartbeat:Connect(function()
        ...
        if os.clock()>=fireAt then
            ...
            isStealing=true; task.delay(0.9,function() isStealing=false end)
            if lagOnSteal then lagSelf() end
            -- Try RF bypass first, then tool activate
            local char=lp.Character
            if FLASH_RF then pcall(function() FLASH_RF:InvokeServer() end)
            else local tool=char and char:FindFirstChildOfClass("Tool"); if tool then pcall(function() tool:Activate() end) end end
...
local speedEnabled=stealSpdOn; local stealingSpeed=STEAL_SPD_VAL; local isSp=false
local function applySpeed()
    ...
    if hum.MoveDirection.Magnitude>0 then local spd=isSp and stealingSpeed or 30; root.Velocity=Vector3.new(hum.MoveDirection.X*spd,root.Velocity.Y,hum.MoveDirection.Z*spd) end
end

-- Auto Grab v2 (from Phantom Grab v2 by r9qbx, no GUI, toggleable)
local grabEnabled=Cfg.grabOn; local activeHolds={}
local GRAB_DIST=12; local HOLD_DUR_GRAB=1.5
local function getAllGrabPrompts()
    local result={}; local char=lp.Character; local root=char and (char:FindFirstChild("HumanoidRootPart") or char:FindFirstChild("UpperTorso")); if not root then return result end
    local plots=workspace:FindFirstChild("Plots"); if not plots then return result end
    for _,plot in ipairs(plots:GetChildren()) do
...
lp.CharacterAdded:Connect(function()
    ...
    activeHolds={}; isStealing=false
end)
RunService.Heartbeat:Connect(function()
    if not grabEnabled then return end
    local root=lp.Character and (lp.Character:FindFirstChild("HumanoidRootPart") or lp.Character:FindFirstChild("UpperTorso")); if not root then return end
    local prompts=getAllGrabPrompts(); local seen={}
    for _,entry in ipairs(prompts) do
        ...
        if not activeHolds[p] then
            ...
            pcall(function() p.Enabled=true end); pcall(function() p:InputHoldBegin() end)
        end
        ...
        if (dist<=GRAB_DIST and progress>=0.92) or progress>=1.05 then
            ...
            if not flashEnabled then
                -- Grab fires steal (flash TP disabled)
                isStealing=true
                pcall(function() fireproximityprompt(p) end); pcall(function() fireproximityprompt(p,10000) end)
                pcall(function() local conns=getconnections(p.Triggered); if conns then for _,c in ipairs(conns) do if c.Function then task.spawn(c.Function) end end end end)
            end
            -- If flashEnabled, Flash TP handles it; grab just ends hold
            task.spawn(function()
                task.wait(0.1); pcall(function() p:InputHoldEnd() end); activeHolds[p]=nil
                task.wait(0.3); isStealing=false
            end)
        end
    end
    for p,data in pairs(activeHolds) do
        if not seen[p] then if not data.fired then pcall(function() p:InputHoldEnd() end) end; activeHolds[p]=nil end
    end
end)

local antiSteal=false; local autoKickBase=false; local lastPunish={}; local myStealPrompts={}
task.spawn(function() while task.wait(2) do myStealPrompts={}
    if myPlotRef then for _,obj in ipairs(myPlotRef:GetDescendants()) do if obj:IsA("ProximityPrompt") and obj.ActionText=="Steal" and obj.Enabled then table.insert(myStealPrompts,obj) end end end
end end)
local function punish(p)
    if not ADM_REMOTE or not p or p==lp then return end
    local uid=p.UserId; local now=tick(); if lastPunish[uid] and now-lastPunish[uid]<0.5 then return end; lastPunish[uid]=now
...
local function findThief()
    local searchPos=stealHitbox and stealHitbox.Position; if not searchPos then local hrp=lp.Character and lp.Character:FindFirstChild("HumanoidRootPart"); searchPos=hrp and hrp.Position end; if not searchPos then return nil end
    local best,bd=nil,math.huge
    for _,p in ipairs(Players:GetPlayers()) do if p~=lp and p.Character then local hrp=p.Character:FindFirstChild("HumanoidRootPart"); if hrp then local d=(hrp.Position-searchPos).Magnitude; if d<bd then bd=d;best=p end end end end
    return best
end
local function isPromptOnMyBase(prompt) if not myPlotRef then return false end; local a=prompt.Parent; while a and a~=workspace do if a==myPlotRef then return true end; a=a.Parent end; return false end
pcall(function()
    ...
    hookfunction(fireproximityprompt,newcclosure(function(prompt,...)
        if antiSteal and (prompt.ActionText or ""):lower():find("steal") and isPromptOnMyBase(prompt) then local t=findThief(); if t then punish(t) end end
        return old(prompt,...)
    end))
end)
task.spawn(function() pcall(function()
...
-- Admin detection (scans for AP gamepass silently)
local adminWarnLbl=nil
task.spawn(function() task.wait(5)
    pcall(function()
        if ADMIN_GP_ID==0 then local shop=workspace:FindFirstChild("RobuxShop"); if shop then for _,obj in ipairs(shop:GetDescendants()) do if obj.Name:lower():find("admin") then local id=obj:GetAttribute("GamepassId") or obj:GetAttribute("GamePassId"); if id and type(id)=="number" and id>0 then ADMIN_GP_ID=id; break end end end end end
    end)
    while true do task.wait(25)
        if not adminWarnLbl or not adminWarnLbl.Parent then break end
        local adminFound={}; local total=#Players:GetPlayers()
        for _,p in ipairs(Players:GetPlayers()) do if p==lp then continue end; task.wait(0.5)
            local has=false
            if ADMIN_GP_ID>0 then local ok,owns=pcall(function() return MPS:UserOwnsGamePassAsync(p.UserId,ADMIN_GP_ID) end); if ok and owns then has=true end end
            if not has then for _,attr in ipairs({"Admin","AdminLevel","HasAdmin"}) do if p:GetAttribute(attr) then has=true; break end end end
            if has then table.insert(adminFound,p.DisplayName) end
        end
        if adminWarnLbl and adminWarnLbl.Parent then
            if #adminFound>0 then adminWarnLbl.Text="!! "..#adminFound.." w/ AP: "..table.concat(adminFound,", ").." !!"
            else adminWarnLbl.Text="Scanning for AP | "..total.." in server" end
        end
    end
end)
...
RunService.Heartbeat:Connect(function(dt)
    lastDt=dt; local now=tick(); if speedEnabled then applySpeed() end
    for _,cmds in pairs(cmdBtnRefs) do for cmd,ib in pairs(cmds) do if ib and ib.Parent then
        local cd=rowCdEnd[cmd]
        if cd and cd>now then local rem=math.ceil(cd-now); local rs=tostring(rem); if ib.Text~=rs then ib.Text=rs; ib.TextSize=9; ib.TextColor3=T.red; ib.BackgroundColor3=Color3.fromRGB(65,8,8); ib.BackgroundTransparency=0.1 end
        else local icon=CMD_ICONS[cmd] or "?"; if ib.Text~=icon then ib.Text=icon; ib.TextSize=11; ib.TextColor3=T.a1; ib.BackgroundColor3=T.surf2; ib.BackgroundTransparency=0.22 end end
    end end end
end)
...
hdrF.BackgroundColor3=T.panel; hdrF.BackgroundTransparency=0.14; hdrF.BorderSizePixel=0; co(hdrF,9); rotS(hdrF,T.a1); particles(hdrF,4); hdrF.Active=false
lb(hdrF,{Position=UDim2.fromOffset(0,5),Size=UDim2.new(1,0,0,16),Text="AETHEN HUB  v1.1",TextSize=11,Font=Enum.Font.GothamBlack,TextColor3=T.a1,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=4})
lb(hdrF,{Position=UDim2.fromOffset(0,20),Size=UDim2.new(1,0,0,11),Text="r9qbx",TextSize=7,Font=Enum.Font.Gotham,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=4})
local fpsLb=lb(hdrF,{Position=UDim2.fromOffset(0,31),Size=UDim2.new(1,0,0,11),Text="FPS --  PING --ms",TextSize=7,Font=Enum.Font.GothamBold,TextColor3=T.grn,TextXAlignment=Enum.TextXAlignment.Center,ZIndex=4})
task.spawn(function() while task.wait(0.7) do
    if not fpsLb.Parent then break end; local fps=math.min(999,math.floor(1/math.max(lastDt,0.001))); local ping=0
    pcall(function() ping=math.floor(lp:GetNetworkPing()*1000) end)
    fpsLb.Text="FPS "..fps.."  PING "..ping.."ms"; fpsLb.TextColor3=fps<40 and T.red or fps<55 and T.yel or T.grn
end end)

-- Quick action buttons (3 columns)
local function hdrBtn(text,pos,w,tc,bg)
    local b=Instance.new("TextButton",hdrF); b.Size=UDim2.fromOffset(w or 60,15); b.Position=pos
    b.BackgroundColor3=bg or T.surf; b.BackgroundTransparency=0.22; b.Text=text; b.TextColor3=tc or T.a1
    b.Font=Enum.Font.GothamBold; b.TextSize=7; b.BorderSizePixel=0; b.AutoButtonColor=false; co(b,4)
    ms(b,1,T.str,0.5); b.ZIndex=5; return b
end
local grabHdrBtn=hdrBtn("Grab: OFF",UDim2.fromOffset(6,46),62,T.dim)
local function updGrabHdr() grabHdrBtn.Text=grabEnabled and "Grab: ON" or "Grab: OFF"; grabHdrBtn.TextColor3=grabEnabled and T.grn or T.dim
    TweenService:Create(grabHdrBtn,TweenInfo.new(0.1),{BackgroundColor3=grabEnabled and Color3.fromRGB(8,22,12) or T.surf}):Play() end
updGrabHdr(); grabHdrBtn.MouseButton1Click:Connect(function() grabEnabled=not grabEnabled; Cfg.grabOn=grabEnabled; save(); updGrabHdr() end)

local rsPnlBtn=hdrBtn("Reset Panels",UDim2.fromOffset(72,46),68,T.dim)
local discordBtn=hdrBtn("Discord",UDim2.fromOffset(144,46),60,T.a2,T.surf)
...
discordBtn.MouseButton1Click:Connect(function()
    ...
    discordBtn.Text="Copied!"; discordBtn.TextColor3=T.grn
...
ragBtn.MouseButton1Click:Connect(function() if ADM_REMOTE then pcall(function() ADM_REMOTE:InvokeServer(ADM_UUID,lp,"ragdoll") end); rowCdEnd["ragdoll"]=tick()+(ROW_CD["ragdoll"] or 30) end end)
...
prBtn.MouseButton1Click:Connect(function()
    task.spawn(function()
        local char=lp.Character; if char then pcall(function()
            local hum=char:FindFirstChildOfClass("Humanoid"); if not hum then return end
        ...
        task.wait(0.5); if ADM_REMOTE then pcall(function() ADM_REMOTE:InvokeServer(ADM_UUID,lp,"ragdoll") end); rowCdEnd["ragdoll"]=tick()+(ROW_CD["ragdoll"] or 30) end
    end)
end)

-- Admin warning text
local warnF=Instance.new("Frame",SG); warnF.Size=UDim2.fromOffset(300,18); warnF.Position=UDim2.new(0.5,-150,1,-42)
warnF.BackgroundColor3=Color3.fromRGB(10,3,3); warnF.BackgroundTransparency=0.18; warnF.BorderSizePixel=0; co(warnF,5); ms(warnF,1,T.red,0.4); warnF.Active=false; warnF.ZIndex=10
adminWarnLbl=lb(warnF,{Size=UDim2.new(1,0,1,0),Text="Scanning for AP...",TextSize=8,Font=Enum.Font.GothamBlack,TextColor3=T.dim,ZIndex=11})
task.spawn(function() while adminWarnLbl and adminWarnLbl.Parent do
    TweenService:Create(adminWarnLbl,TweenInfo.new(0.45,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{TextTransparency=0.72}):Play(); task.wait(0.45)
    if not adminWarnLbl or not adminWarnLbl.Parent then break end
    TweenService:Create(adminWarnLbl,TweenInfo.new(0.45,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{TextTransparency=0}):Play(); task.wait(0.45)
end end)

-- Flash TP panel with MAIN/EXTRAS tabs
local FP_W=148; local CONT_H=192; local FP_H=21+20+CONT_H+4
...
tabRow.BackgroundColor3=T.surf2; tabRow.BackgroundTransparency=0.1; tabRow.BorderSizePixel=0
local tabMainBtn=Instance.new("TextButton",tabRow); tabMainBtn.Size=UDim2.new(0.5,0,1,0); tabMainBtn.BackgroundColor3=T.a1; tabMainBtn.Text="MAIN"; tabMainBtn.Font=Enum.Font.GothamBlack; tabMainBtn.TextSize=8; tabMainBtn.TextColor3=T.bg; tabMainBtn.BorderSizePixel=0; tabMainBtn.AutoButtonColor=false; co(tabMainBtn,0)
local tabExtraBtn=Instance.new("TextButton",tabRow); tabExtraBtn.Size=UDim2.new(0.5,0,1,0); tabExtraBtn.Position=UDim2.new(0.5,0,0,0); tabExtraBtn.BackgroundColor3=T.surf2; tabExtraBtn.Text="EXTRAS"; tabExtraBtn.Font=Enum.Font.GothamBlack; tabExtraBtn.TextSize=8; tabExtraBtn.TextColor3=T.dim; tabExtraBtn.BorderSizePixel=0; tabExtraBtn.AutoButtonColor=false; co(tabExtraBtn,0)
local cont=Instance.new("Frame",FP); cont.Size=UDim2.fromOffset(FP_W,CONT_H); cont.Position=UDim2.fromOffset(0,43); cont.BackgroundTransparency=1; cont.BorderSizePixel=0; cont.ClipsDescendants=true
...
local function setTab(isMain)
    tabMainBtn.BackgroundColor3=isMain and T.a1 or T.surf2; tabMainBtn.TextColor3=isMain and T.bg or T.dim
    tabExtraBtn.BackgroundColor3=not isMain and T.a1 or T.surf2; tabExtraBtn.TextColor3=not isMain and T.bg or T.dim
    pg1.Visible=isMain; pg2.Visible=not isMain
end
tabMainBtn.MouseButton1Click:Connect(function() setTab(true) end)
tabExtraBtn.MouseButton1Click:Connect(function() setTab(false) end)
setTab(true)

-- MAIN page
local lagRow=Instance.new("Frame",pg1); lagRow.Size=UDim2.fromOffset(FP_W-10,21); lagRow.Position=UDim2.fromOffset(5,4); lagRow.BackgroundColor3=T.surf; lagRow.BackgroundTransparency=0.28; lagRow.BorderSizePixel=0; co(lagRow,6); ms(lagRow,1,T.str,0.55)
local lagLbl=lb(lagRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Lag On Steal",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local lagTrack=Instance.new("Frame",lagRow); lagTrack.Size=UDim2.fromOffset(25,12); lagTrack.Position=UDim2.new(1,-29,0.5,-6); lagTrack.BackgroundColor3=T.surf2; lagTrack.BorderSizePixel=0; co(lagTrack,12)
local lagKnob=Instance.new("Frame",lagTrack); lagKnob.Size=UDim2.fromOffset(8,8); lagKnob.Position=UDim2.new(0,2,0.5,-4); lagKnob.BackgroundColor3=T.a2; lagKnob.BorderSizePixel=0; co(lagKnob,8)
local lagBtnR=Instance.new("TextButton",lagRow); lagBtnR.Size=UDim2.new(1,0,1,0); lagBtnR.BackgroundTransparency=1; lagBtnR.Text=""
local lagRS=lagRow:FindFirstChildOfClass("UIStroke")
local function setLag(v) lagOnSteal=v
...
local potRow=Instance.new("Frame",pg1); potRow.Size=UDim2.fromOffset(FP_W-10,21); potRow.Position=UDim2.fromOffset(5,28); potRow.BackgroundColor3=T.surf; potRow.BackgroundTransparency=0.28; potRow.BorderSizePixel=0; co(potRow,6); ms(potRow,1,T.str,0.55)
local potLbl=lb(potRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Giant Potion",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local potTrack=Instance.new("Frame",potRow); potTrack.Size=UDim2.fromOffset(25,12); potTrack.Position=UDim2.new(1,-29,0.5,-6); potTrack.BackgroundColor3=T.surf2; potTrack.BorderSizePixel=0; co(potTrack,12)
local potK=Instance.new("Frame",potTrack); potK.Size=UDim2.fromOffset(8,8); potK.Position=UDim2.new(0,2,0.5,-4); potK.BackgroundColor3=T.a2; potK.BorderSizePixel=0; co(potK,8)
local potBtnR=Instance.new("TextButton",potRow); potBtnR.Size=UDim2.new(1,0,1,0); potBtnR.BackgroundTransparency=1; potBtnR.Text=""
local potRS=potRow:FindFirstChildOfClass("UIStroke")
local function updPot()
    TweenService:Create(potRow,TweenInfo.new(0.12),{BackgroundColor3=potionOn and Color3.fromRGB(10,10,18) or T.surf,BackgroundTransparency=potionOn and 0.15 or 0.28}):Play()
...
updPot(); potBtnR.MouseButton1Click:Connect(function() potionOn=not potionOn; Cfg.potionOn=potionOn; save(); updPot() end)

local sep1=Instance.new("Frame",pg1); sep1.Size=UDim2.new(1,-10,0,1); sep1.Position=UDim2.fromOffset(5,52); sep1.BackgroundColor3=T.str; sep1.BackgroundTransparency=0.68; sep1.BorderSizePixel=0

local togRow=Instance.new("Frame",pg1); togRow.Size=UDim2.fromOffset(FP_W-10,21); togRow.Position=UDim2.fromOffset(5,56); togRow.BackgroundColor3=T.surf; togRow.BackgroundTransparency=0.28; togRow.BorderSizePixel=0; co(togRow,6)
local togRS=ms(togRow,1,T.str,0.55)
local togLbl=lb(togRow,{Position=UDim2.new(0,7,0,0),Size=UDim2.new(1,-38,1,0),Text="Flash TP",TextSize=8,Font=Enum.Font.GothamBold,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local pill=Instance.new("Frame",togRow); pill.Size=UDim2.fromOffset(25,12); pill.Position=UDim2.new(1,-29,0.5,-6); pill.BackgroundColor3=T.surf2; pill.BorderSizePixel=0; co(pill,12)
local knob=Instance.new("Frame",pill); knob.Size=UDim2.fromOffset(8,8); knob.Position=UDim2.new(0,2,0.5,-4); knob.BackgroundColor3=T.a2; knob.BorderSizePixel=0; co(knob,8)
...
local function updFl()
    TweenService:Create(togRow,TweenInfo.new(0.12),{BackgroundColor3=flashEnabled and Color3.fromRGB(10,10,18) or T.surf,BackgroundTransparency=flashEnabled and 0.15 or 0.28}):Play()
...
local trigL=lb(pg1,{Position=UDim2.fromOffset(5,81),Size=UDim2.fromOffset(FP_W-10,11),Text="Trigger: "..Cfg.triggerChance.."%",TextSize=8,TextColor3=T.dim,TextXAlignment=Enum.TextXAlignment.Left})
local slBg=Instance.new("Frame",pg1); slBg.Size=UDim2.fromOffset(FP_W-10,5); slBg.Position=UDim2.fromOffset(5,95); slBg.BackgroundColor3=T.surf; slBg.BorderSizePixel=0; co(slBg,3)
...
local camBusy=false; local camBtn=btn(pg1,"AUTO ALLIGN",UDim2.fromOffset(FP_W-10,19),UDim2.fromOffset(5,104),T.surf); camBtn.TextSize=9; camBtn.TextColor3=T.a1
do local s=camBtn:FindFirstChildOfClass("UIStroke"); if s then s.Color=T.a1;s.Transparency=0.28 end end
...
local rstBtn=btn(pg1,"Instant Reset",UDim2.fromOffset(FP_W-10,17),UDim2.fromOffset(5,127),Color3.fromRGB(40,6,6)); rstBtn.TextSize=8; rstBtn.TextColor3=T.red
...
ssB.MouseButton1Click:Connect(function() stealSpdOn=not stealSpdOn; isSp=stealSpdOn; speedEnabled=stealSpdOn; ssSet(stealSpdOn); Cfg.stealSpdOn=stealSpdOn; save() end)
if stealSpdOn then ssSet(true); isSp=true; speedEnabled=true end
numRow(pg2,"Speed",STEAL_SPD_VAL,10,60,26,FP_W,function(v) STEAL_SPD_VAL=v; stealingSpeed=v; Cfg.stealSpdVal=v; save() end)
local sep3=Instance.new("Frame",pg2); sep3.Size=UDim2.new(1,-10,0,1); sep3.Position=UDim2.fromOffset(5,48); sep3.BackgroundColor3=T.str; sep3.BackgroundTransparency=0.68; sep3.BorderSizePixel=0
local brainESPB,brainESPSet=pillR(pg2,"Brainrot ESP",52,T.a1); brainESPB.MouseButton1Click:Connect(function() if brainrotESPEnabled then stopBrainrotESP(); brainESPSet(false) else startBrainrotESP(); brainESPSet(true) end end)
local friendESPB,friendESPSet=pillR(pg2,"Friends ESP",75,T.grn); friendESPB.MouseButton1Click:Connect(function() if friendESPEnabled then stopFriendESP(); friendESPSet(false) else startFriendESP(); friendESPSet(true) end end)
local xrB,xrSet=pillR(pg2,"Base Xray",98,T.red); xrB.MouseButton1Click:Connect(function() if xrayOn then disableXray(); xrSet(false) else enableXray(); xrSet(true) end end)

-- Aethens panel
local AP_W2=130; local AP_CH=72; local AP_H2=21+AP_CH+6
...
local kickB=apB("Kick Self",T.red,Color3.fromRGB(30,6,6)); kickB.MouseButton1Click:Connect(function() kickB.Text="..."; lp:Kick(DISCORD_LINK) end)
...
local pScroll=Instance.new("ScrollingFrame",ADMpan); pScroll.Size=UDim2.fromOffset(ADM_W-8,0); pScroll.Position=UDim2.fromOffset(4,22)
pScroll.BackgroundColor3=T.bg; pScroll.BackgroundTransparency=0.65; pScroll.BorderSizePixel=0; pScroll.ScrollBarThickness=2; pScroll.ScrollBarImageColor3=T.a1; pScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; pScroll.CanvasSize=UDim2.new(0,0,0,0); co(pScroll,4)
...
local function resADM()
    if admFolded then return end; local ch=math.min(pLayout.AbsoluteContentSize.Y+4,120)
    pScroll.Size=UDim2.fromOffset(ADM_W-8,ch); ADMpan.Size=UDim2.fromOffset(ADM_W,22+ch+3)
end
...
local function refreshPlayers()
    ...
    for _,p in ipairs(Players:GetPlayers()) do
        ...
        local card=Instance.new("Frame",pScroll); card.Name=p.Name; card.Size=UDim2.new(1,-4,0,30); card.BackgroundColor3=T.surf; card.BackgroundTransparency=0.26; card.BorderSizePixel=0; co(card,5)
        local cs=ms(card,1,T.str,0.55)
        local cz=Instance.new("TextButton",card); cz.Size=UDim2.fromOffset(CSTART,30); cz.BackgroundTransparency=1; cz.Text=""; cz.AutoButtonColor=false; cz.Parent=card
        local ava=Instance.new("ImageLabel",card); ava.Size=UDim2.fromOffset(18,18); ava.Position=UDim2.new(0,3,0.5,-9); ava.BackgroundColor3=T.surf2; ava.BorderSizePixel=0; co(ava,9)
        ...
        for i,def in ipairs(RCMDS) do
            ...
            cmdBtnRefs[p.UserId][def.cmd]=ib; local pd=p; local dc=def.cmd; ib.MouseButton1Click:Connect(function() fireCmd(pd,dc) end)
        end
        local pd=p
...
-- Base Protector panel (renamed, fixed detection)
local BP_W=136; local BP_CH=4*23+6; local BP_H=21+BP_CH+6
local BP_defX=math.floor(vp.X*0.5-BP_W*0.5); local BP_defY=math.floor(vp.Y*0.5-BP_H*0.5)
local BPpan,_,bpMin=mkP(BP_defX,BP_defY,BP_W,BP_H,"Base Protector",T.a1,"pos_BP")
local bpC=mkF(BPpan,21,BP_CH+6,bpMin); local pyy=4
local asB,asSet=pillR(bpC,"Anti Steal",pyy,T.a1); pyy=pyy+23; asB.MouseButton1Click:Connect(function() antiSteal=not antiSteal; asSet(antiSteal) end)
local akB,akSet=pillR(bpC,"Auto Kick",pyy,T.red); pyy=pyy+23; akB.MouseButton1Click:Connect(function() autoKickBase=not autoKickBase; akSet(autoKickBase) end)
local aiB,aiSet=pillR(bpC,"Anti Intruder",pyy,T.a2); pyy=pyy+23
do
    local aiOn=false; local carpetSpam={}
    local function CARPET(p) local char=p.Character; if not char then return false end; for _,item in ipairs(char:GetChildren()) do if ({["Flying Carpet"]=1,["Witch's Broom"]=1,["Santa's Sleigh"]=1,["Cupid's Wings"]=1})[item.Name] then return true end end; return false end
    local aiConn=nil
    aiB.MouseButton1Click:Connect(function() aiOn=not aiOn; aiSet(aiOn)
        if aiOn and not aiConn then aiConn=RunService.Heartbeat:Connect(function()
            if not aiOn or not stealHitbox or not ADM_REMOTE then return end
            local cf,sz=stealHitbox.CFrame,stealHitbox.Size; local hx,hz=sz.X*0.5,sz.Z*0.5
            for _,p in ipairs(Players:GetPlayers()) do if p~=lp and p.Character and CARPET(p) then
                ...
                if math.abs(rel.X)<=hx and math.abs(rel.Z)<=hz then
                    local uid=p.UserId; if not carpetSpam[uid] then carpetSpam[uid]=true
        ...
        end) elseif not aiOn and aiConn then aiConn:Disconnect(); aiConn=nil end
    end)
end
local arB,arSet=pillR(bpC,"Anti Ragdoll",pyy,T.grn); arSet(true)
arB.MouseButton1Click:Connect(function() end)  -- always on, just shows status
...
rsPnlBtn.MouseButton1Click:Connect(function()
    ...
    local yesB=btn(popup,"Yes",UDim2.fromOffset(76,22),UDim2.fromOffset(10,36),Color3.fromRGB(10,32,10)); yesB.TextColor3=T.grn; do local s=yesB:FindFirstChildOfClass("UIStroke"); if s then s.Color=T.grn;s.Transparency=0.3 end end
    local noB=btn(popup,"No",UDim2.fromOffset(76,22),UDim2.fromOffset(102,36),Color3.fromRGB(32,8,8)); noB.TextColor3=T.red; do local s=noB:FindFirstChildOfClass("UIStroke"); if s then s.Color=T.red;s.Transparency=0.3 end end
    ...
    yesB.MouseButton1Click:Connect(function()
        ...
        FP.Position=UDim2.fromOffset(FP_defX,FP_defY); APan.Position=UDim2.fromOffset(AP_defX,AP_defY)
        ADMpan.Position=UDim2.fromOffset(ADM_defX,ADM_defY); BPpan.Position=UDim2.fromOffset(BP_defX,BP_defY)
        popup:Destroy()
    end)
end)
