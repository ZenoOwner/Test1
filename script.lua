...
local function mkESP(plr)
    ...
    bg.BackgroundTransparency=0.2 bg.BorderSizePixel=0 co(bg,5) stk(bg,C.ELEC,1.3)
...
local function equipCarpet()
    ...
    for _,t in ipairs(bp:GetChildren()) do
        if t:IsA("Tool") and (t.Name:lower():find("carpet") or t.Name:lower():find("flying")) then
...
local function makeRow(parent,label,order,accent)
    accent=accent or C.LIME
    local row=Instance.new("Frame",parent) row.Size=UDim2.new(1,0,0,28) row.BackgroundColor3=C.CARD2
...
local function makeBtn(parent,txt,col,order)
    ...
    btn.BackgroundTransparency=0.15 btn.BorderSizePixel=0 btn.Text=txt btn.Font=Enum.Font.GothamBold
    btn.TextSize=9 btn.TextColor3=col btn.AutoButtonColor=false btn.LayoutOrder=order or 99
...
Players.PlayerRemoving:Connect(function(p)
    if selectedTarget==p then selectedTarget=nil end refreshLists()
end)
for _,p in ipairs(Players:GetPlayers()) do if p~=lp then
