...
local function makePanel(name,w,h,col,defX,defY,key)
    ...
    local mb=btn(hdr,"â”€",UDim2.fromOffset(14,13),UDim2.new(1,-17,0.5,-6.5),C.surf2,C.dim,8); mb.BackgroundTransparency=0.6
...
local fpsLb=lbl(fpsF,{Size=UDim2.new(1,0,1,0);Text="FPS --  Â·  PING -- ms";TextSize=7;TextColor3=C.dim})
local lastDt=1/60
...
settingsBtn.BackgroundTransparency=0.2; settingsBtn.Text="âš™"; settingsBtn.TextSize=15
...
local ftIcon=lbl(ftIconBox,{Size=UDim2.new(1,0,1,0);Text="âš¡";TextSize=16;Font=Enum.Font.GothamBlack;TextColor3=C.accent})
...
local ftSub=lbl(ftBtn,{Position=UDim2.new(0,42,0,21);Size=UDim2.new(1,-90,0,12);Text="off â€” toggle to enable";TextSize=7;Font=Enum.Font.Gotham;TextColor3=C.str})
-- pill
...
local function setFlashVis(v)
    ...
    ftSub.Text=v and "on â€” fires at "..math.floor(sliderValue*100).."%"  or "off â€” toggle to enable"
...
local function setSlider(px)
    ...
    if flashEnabled then ftSub.Text="on â€” fires at "..math.floor(rel*100).."%" end
end
...
bigBtn.Text="âš¡  FLASH GRAB"; bigBtn.TextColor3=C.white; bigBtn.Font=Enum.Font.GothamBlack; bigBtn.TextSize=15
...
settingsBtn.MouseButton1Click:Connect(function()
    ...
    settingsBtn.Text=settingsOpen and "âœ•" or "âš™"
...
local rejBtn=btn(actC,"â†º  REJOIN",UDim2.new(1,-8,0,20),UDim2.new(0,0,0,0),C.surf2,C.white,9)
local kickBtn=btn(actC,"âœ•  KICK SELF",UDim2.new(1,-8,0,20),UDim2.new(0,0,0,0),C.surf2,C.red,9)
...
local apMb=btn(apHdr,"â”€",UDim2.fromOffset(14,13),UDim2.new(1,-17,0.5,-6.5),C.surf2,C.dim,8); apMb.BackgroundTransparency=0.6
dragF(AP,apHdr,"ap")
...
apMb.MouseButton1Click:Connect(function()
    apFolded=not apFolded; apMb.Text=apFolded and "+" or "â”€"; pScroll.Visible=not apFolded
...
local RCMDS={{cmd="ragdoll",icon="ðŸƒ"},{cmd="balloon",icon="ðŸŽˆ"},{cmd="tiny",icon="ðŸœ"},{cmd="jail",icon="ðŸ”“"},{cmd="rocket",icon="ðŸš€"}}
...
bigBtn.MouseButton1Click:Connect(function()
    ...
    if not prompt or not isPromptVisible(prompt) then
        showStatus("ðŸš« No brainrot found",C.red,1.8)
        return
    end

    showStatus("âš¡ Grabbing...",C.accent,2)
...
-- Anti Steal (Phantom Flash V5 style â€” thief closest to hitbox)
...
local function mkESP(plr)
    ...
    task.spawn(function() task.wait(0.8); local hasAP=false
        ...
        if hasAP then apF.BackgroundColor3=Color3.fromRGB(5,18,8); stk(apF,0.7,C.grn,0); apL.TextColor3=C.grn; apL.Text="APâœ“"
        else apF.BackgroundColor3=Color3.fromRGB(18,5,5); stk(apF,0.7,C.red,0); apL.TextColor3=C.red; apL.Text="APâœ—" end
    end)
end
...
local function showAlert()
    ...
    lbl(af,{Size=UDim2.new(1,0,1,0);Text="âš   BASE EXPIRES IN 5s!";TextSize=8;Font=Enum.Font.GothamBlack;TextColor3=C.red})
