...
local function getPlotOwnerPlayer()
    local sel = _G._SP_SelectedBrainrot
...
local CONFIG_PATH = "ShadePvp_config.json"

local function SP_SerializeConfig()
...
local function SP_ParseConfig(str)
    local pos = 1
...
local function SP_SaveConfig()
    pcall(writefile, CONFIG_PATH, SP_SerializeConfig())
end

local function SP_LoadConfig()
    local ok, data = pcall(function() return SP_ParseConfig(readfile(CONFIG_PATH)) end)
...
local function SP_ApplyLoadedKeybinds()
...
T = {
    ...
    CardHover   = Color3.fromRGB(235, 235, 235),
    ...
    TabInact    = Color3.fromRGB(150, 150, 150),
    TrackOn     = Color3.fromRGB(20,  20,  20),
    TrackOff    = Color3.fromRGB(200, 200, 200),
...
pcall(function()
    if game.CoreGui:FindFirstChild("ShadePvp") then
        game.CoreGui.ShadePvp:Destroy()
    end
end)
...
do
    ...
    ShowToggleNotification = function(toggleName, enabled)
        ...
        toast.Parent                 = shadow
        Corner(toast, 10)
        local _stroke = Stroke(toast, Color3.fromRGB(180, 180, 180), 1); _stroke.Transparency = 1
        ...
        pill.Parent                 = toast
        Corner(pill, 2)
        local nameLabel = Label(toast, toggleName, 11, Color3.fromRGB(20, 20, 20), Enum.Font.GothamBold)
        ...
        barTrack.BackgroundColor3 = Color3.fromRGB(210, 210, 210); barTrack.BorderSizePixel = 0
        barTrack.ZIndex = 101; barTrack.Parent = toast
        local barFill = Instance.new("Frame")
        barFill.Size = UDim2.new(1, 0, 1, 0); barFill.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
...
WinStroke.Color           = Color3.fromRGB(180, 180, 180)
...
HdrFill.BackgroundTransparency = 0
...
HdrLine.BackgroundColor3 = Color3.fromRGB(200, 200, 200)
...
Dot.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
...
TitleLbl.Text     = '<font color="rgb(245,245,245)">Shade</font><font color="rgb(255,255,255)">Pvp</font>'
...
do
    ...
    _G._SP_ResetBtnEntry = makeActionBtn("RESET", 2, function()
    ...
    _G._SP_FlashBtnEntry = makeActionBtn("FLASH", 3, function()
        local lp  = Players.LocalPlayer
        local chr = lp and lp.Character
        local hrp = chr and chr:FindFirstChild("HumanoidRootPart")
        ...
        local selected = _G._SP_SelectedBrainrot
    ...
    _G._SP_BlockBtnEntry = makeActionBtn("BLOCK", 4, function()
...
CreateButton = function(parent, name, desc, cb)
    ...
    runLbl.BackgroundColor3      = Color3.fromRGB(220, 220, 220)
    ...
    card.MouseEnter:Connect(function()
        Tween(card,    F, {BackgroundColor3 = T.CardHover})
        Tween(cStroke, F, {Color = T.BorderHover})
        Tween(runLbl,  F, {BackgroundColor3 = Color3.fromRGB(210, 210, 210)})
    end)
    card.MouseLeave:Connect(function()
        Tween(card,    F, {BackgroundColor3 = T.Card})
        Tween(cStroke, F, {Color = T.Border})
        Tween(runLbl,  F, {BackgroundColor3 = Color3.fromRGB(220, 220, 220)})
    end)
    local btn = Instance.new("Frame")
    btn.Size                = UDim2.new(1, 0, 1, 0)
    btn.BackgroundTransparency = 1
    ...
    local function fireButton()
        ...
        task.delay(0.35, function()
            Tween(bar,       M, {BackgroundColor3 = T.TrackOff})
            Tween(runLbl,    M, {BackgroundColor3 = Color3.fromRGB(220, 220, 220)})
...
do
    ...
    local AnimalsData   = _G._SP_AG_AnimalsData
    local AnimalsShared = _G._SP_AG_AnimalsShared
    local NumberUtils   = _G._SP_AG_NumberUtils
    ...
    local RequestData   = _G._SP_AG_SyncRemotes and _G._SP_AG_SyncRemotes.requestData
    ...
    local PlotSyncCaches = (_G._SP_AG_PlotSync and _G._SP_AG_PlotSync.caches) or {}
    ...
    local function buildAnimalCard(rec, index)
        ...
        if not _G._SP_CardSelectFns then _G._SP_CardSelectFns = {} end
        _G._SP_CardSelectFns[uid] = programmaticSelect
    ...
    local function autoSelectBest()
        local results = _G._SP_LastAnimalScan
        ...
        if _G._SP_CardSelectFns and _G._SP_CardSelectFns[uid] then
            pcall(_G._SP_CardSelectFns[uid])
        end
    end
    _G._SP_AutoSelectBest = autoSelectBest
...
do
    ...
    _G._SP_DoSelectedReset = doSelectedReset
...
do
    ...
    local function tryAutoFlash()
        ...
        if not _G._SP_SelectedBrainrot then
        ...
        task.spawn(function()
            ...
            if _G._SP_DoSelectedReset then
                pcall(_G._SP_DoSelectedReset)
            end
            ...
            if not _G._SP_SelectedBrainrot then _afPending = false; return end

            pcall(ShowToggleNotification, "Auto Flash: flashing!", true)
            if _G._SP_FlashBtnEntry then
                pcall(_G._SP_FlashBtnEntry.fire)
            end
...
CreateToggle(UtilsTab.scroll, "Auto Select Best Brainrot", "Auto-selects the highest gen animal on scan", function(v)
    ...
    pcall(SP_SaveConfig)
    if v and _G._SP_LastAnimalScan and #_G._SP_LastAnimalScan > 0 then
        pcall(_G._SP_AutoSelectBest)
    end
...
do
    ...
    local function fpBuildWindow()
        ...
        hdr.BackgroundColor3 = Color3.fromRGB(240, 240, 240)
        hdr.BorderSizePixel  = 0
        hdr.ZIndex           = 31
        hdr.Parent           = win
        Corner(hdr, 10)
        ...
        hdrFill.BackgroundColor3 = Color3.fromRGB(240, 240, 240)
        hdrFill.BorderSizePixel  = 0
        hdrFill.ZIndex           = 31
        hdrFill.Parent           = hdr
...
if isMobile then
    ...
    LogoBox.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.Touch and _logoTouchStart then
            ...
            _logoTouchStart = nil
            if mag < 12 then
                Tween(LogoBox, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(210, 210, 210)})
...
local HubLabel = Label(Banner, "Shade Pvp", isMobile and 10 or 13, T.White, Enum.Font.GothamBold)
...
local PINGLabel = Label(Banner, "PING: --ms", isMobile and 9 or 12, Color3.fromRGB(100, 100, 100), Enum.Font.GothamBold)
...
StatusBadge.BackgroundColor3 = Color3.fromRGB(230, 230, 230)
...
if isMobile then
    ...
    MobileBar.Name                   = "ShadePvpMobileBar"
    ...
    MobileBar.BackgroundColor3       = Color3.fromRGB(240, 240, 240)
    ...
    local function makeMobileBtn(text, order, accentCol, onFire)
        ...
        local function _fireBtn()
            ...
            task.delay(0.16, function()
                Tween(btn, M, {BackgroundColor3 = Color3.fromRGB(240, 240, 240)})
    ...
    makeMobileBtn("RESET", 1, Color3.fromRGB(70, 70, 70), function()
        if _G._SP_ResetBtnEntry then pcall(_G._SP_ResetBtnEntry.fire) end
    end)
    makeMobileBtn("FLASH", 2, Color3.fromRGB(90, 130, 200), function()
        if _G._SP_FlashBtnEntry then pcall(_G._SP_FlashBtnEntry.fire) end
    end)
    makeMobileBtn("BLOCK", 3, Color3.fromRGB(180, 60, 60), function()
        if _G._SP_BlockBtnEntry then pcall(_G._SP_BlockBtnEntry.fire) end
    end)
end
...
ShowToggleNotification("ShadePvp loaded", true)
