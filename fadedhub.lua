...
do
	local _active = {}
	local NW, NH, NGAP, NPADX, NPADY, NDUR = 170, 38, 5, 8, 8, 2
	local function _sy(i) return -(NPADY + NH + 4 + i*(NH+NGAP)) end
	local function _repo(ti)
		for i, e in ipairs(_active) do
			TweenService:Create(e.shadow, ti, {Position=UDim2.new(0,NPADX-3,1,_sy(i-1))}):Play()
		end
	end
	ShowToggleNotification = function(name, enabled)
		...
		local II = TweenInfo.new(0.32, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		local OI = TweenInfo.new(0.24, Enum.EasingStyle.Quint, Enum.EasingDirection.In)
		local BI = TweenInfo.new(NDUR, Enum.EasingStyle.Linear)
		local FI = TweenInfo.new(0.2,  Enum.EasingStyle.Linear)
		local RI = TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
		local shadow = Instance.new("Frame")
		shadow.Size=UDim2.new(0,NW+6,0,NH+6)
		shadow.Position=UDim2.new(0,-(NW+24),1,_sy(0))
		shadow.BackgroundColor3=Color3.fromRGB(0,0,0)
		shadow.BackgroundTransparency=0.15; shadow.BorderSizePixel=0; shadow.ZIndex=99; shadow.Parent=GUI
		Corner(shadow, 10)
		local toast = Instance.new("Frame")
		toast.Size=UDim2.new(0,NW,0,NH); toast.Position=UDim2.new(0,3,0,3)
		...
		pill.Size=UDim2.new(0,2,0,NH-12); pill.Position=UDim2.new(0,7,0.5,-(NH-12)/2)
		pill.BackgroundColor3=T.Accent; pill.BorderSizePixel=0; pill.ZIndex=101; pill.Parent=toast
		Corner(pill, 2)
		local nl = Label(toast, name, 10, T.White, Enum.Font.GothamBold)
		nl.Size=UDim2.new(1,-20,0,13); nl.Position=UDim2.new(0,15,0,5)
		nl.TextTruncate=Enum.TextTruncate.AtEnd; nl.TextTransparency=1; nl.ZIndex=101
		local sl = Label(toast, enabled and "Enabled" or "Disabled", 9, scol, Enum.Font.Gotham)
		sl.Size=UDim2.new(1,-20,0,10); sl.Position=UDim2.new(0,15,0,19)
		...
		TweenService:Create(shadow,II,{Position=UDim2.new(0,NPADX-3,1,_sy(0))}):Play()
		TweenService:Create(toast,II,{BackgroundTransparency=0}):Play()
		TweenService:Create(_st,II,{Transparency=0.4}):Play()
		...
		task.delay(NDUR+0.15, function()
			...
			TweenService:Create(shadow,OI,{Position=UDim2.new(0,-(NW+24),1,ey)}):Play()
...
local WIN_W = isMobile and 240 or 280
local WIN_H = isMobile and 340 or 360
...
local HDR_H = isMobile and 56 or 64

local Hdr = Instance.new("Frame")
Hdr.Name="Hdr"
Hdr.Size=UDim2.new(1,0,0,HDR_H); Hdr.BackgroundColor3=T.Header
...
lockBtn.MouseButton1Click:Connect(function()
	...
	if _guiLocked then
		lockBtn.Text="🔓"; lockBtn.BackgroundColor3=Color3.fromRGB(120,30,30)
		Tween(lockStroke,F,{Color=Color3.fromRGB(200,50,50)})
	else
		lockBtn.Text="🔒"; lockBtn.BackgroundColor3=T.Card
...
BigBtnRow.Size=UDim2.new(1,-16,0,32); BigBtnRow.Position=UDim2.new(0,8,0,HDR_H+8)
...
local TOTAL_TOP = HDR_H + BIG_BTN_H + 16

local TabBar = Instance.new("Frame")
TabBar.Size=UDim2.new(1,0,0,30); TabBar.Position=UDim2.new(0,0,0,TOTAL_TOP)
...
TBLine.Size=UDim2.new(1,0,0,1); TBLine.Position=UDim2.new(0,0,0,TOTAL_TOP+29)
...
ContentArea.Size=UDim2.new(1,0,1,-(TOTAL_TOP+31)); ContentArea.Position=UDim2.new(0,0,0,TOTAL_TOP+31)
...
local FOLD_TI_OUT = TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
local FOLD_TI_IN  = TweenInfo.new(0.28, Enum.EasingStyle.Quart, Enum.EasingDirection.In)
local FULL_H = WIN_H
local FOLD_H = HDR_H + BIG_BTN_H + 18
...
foldBtn.MouseButton1Click:Connect(function()
	...
	if _guiFolded then
		...
		TweenService:Create(Win, FOLD_TI_IN, {Size=UDim2.new(0,WIN_W,0,FOLD_H)}):Play()
	else
		foldBtn.Text = "▼"
		TweenService:Create(Win, FOLD_TI_OUT, {Size=UDim2.new(0,WIN_W,0,FULL_H)}):Play()
...
closeBtn.MouseButton1Click:Connect(function()
	if not _minimized then
		...
		_miniFrame.Size = UDim2.new(0,76,0,26)
		_miniFrame.Position = UDim2.new(0,8,0.5,-13)
		_miniFrame.BackgroundColor3 = T.Card
		_miniFrame.BorderSizePixel = 0
		_miniFrame.Text = "Vertex Pvp"
		_miniFrame.TextSize = 10
		_miniFrame.Font = Enum.Font.GothamBold
		_miniFrame.TextColor3 = T.White
		_miniFrame.ZIndex = 50
		_miniFrame.AutoButtonColor = false
		_miniFrame.Parent = GUI
		Corner(_miniFrame, 7)
		Stroke(_miniFrame, T.AccentBright, 1.2)
		local _mts
		...
		_miniFrame.InputEnded:Connect(function(inp)
			if (inp.UserInputType==Enum.UserInputType.Touch or inp.UserInputType==Enum.UserInputType.MouseButton1) and _mts then
...
local function makeActionBtnEntry(label, configKey, onClick)
	...
	if saved then
		...
		if ok2 and kc then entry.keyCode=kc end
	end
...
local function doFlash()
	...
	if not myBase then pcall(ShowToggleNotification,"Flash: cant find your base",false); return end
	...
	if not selected then pcall(ShowToggleNotification,"Flash: pick a brainrot first",false); return end
	local slot = tonumber(selected.slot)
	if not slot then pcall(ShowToggleNotification,"Flash: no slot found",false); return end
	local entry = podiumCFs[slot]
	if not entry or not entry.cam then
		pcall(ShowToggleNotification,"Flash: podium "..tostring(slot).." not set up yet",false); return
	end
	...
	task.spawn(function()
		...
		if not flashTool then pcall(ShowToggleNotification,"Flash: tool not in inventory",false); return end
	...
	pcall(ShowToggleNotification, "Flash fired", true)
end
...
blockBtn.MouseButton1Click:Connect(function()
	...
	else pcall(ShowToggleNotification,"No players nearby",false) end
end)
...
CreateSection = function(parent, title)
	local f=Instance.new("Frame"); f.Size=UDim2.new(1,-12,0,22); f.BackgroundTransparency=1; f.Parent=parent
	local lw=#title*6+6
...
CreateToggle = function(parent, name, desc, cb)
	...
	bar.Position=UDim2.new(0,0,0,7); bar.BackgroundColor3=T.TrackOff; bar.BorderSizePixel=0; bar.ZIndex=2; bar.Parent=card; Corner(bar,2)
	local nameY=hasDesc and 8 or (cardH/2-7)
	local nl=Label(card,name,isMobile and 10 or 12,T.White,Enum.Font.GothamMedium)
	nl.Size=UDim2.new(1,-96,0,14); nl.Position=UDim2.new(0,10,0,nameY); nl.ZIndex=2; nl.TextTruncate=Enum.TextTruncate.AtEnd
	if hasDesc then
		local dl=Label(card,desc,isMobile and 8 or 10,T.Dim,Enum.Font.Gotham)
		dl.Size=UDim2.new(1,-96,0,12); dl.Position=UDim2.new(0,10,0,nameY+16); dl.ZIndex=2; dl.TextTruncate=Enum.TextTruncate.AtEnd
	end
	local track=Instance.new("Frame"); track.Size=UDim2.new(0,36,0,20)
	track.Position=UDim2.new(1,-46,0.5,-10); track.BackgroundColor3=T.TrackOff
...
CreateButton = function(parent, name, desc, cb)
	...
	nl2.Size=UDim2.new(1,-96,0,14); nl2.Position=UDim2.new(0,10,0,nameY); nl2.ZIndex=2; nl2.TextTruncate=Enum.TextTruncate.AtEnd
	if hasDesc then
		local dl2=Label(card,desc,isMobile and 8 or 10,T.Dim,Enum.Font.Gotham)
		dl2.Size=UDim2.new(1,-96,0,12); dl2.Position=UDim2.new(0,10,0,nameY+16); dl2.ZIndex=2; dl2.TextTruncate=Enum.TextTruncate.AtEnd
	end
	local runLbl=Instance.new("TextLabel"); runLbl.Size=UDim2.new(0,46,0,22)
	runLbl.Position=UDim2.new(1,-54,0.5,-11); runLbl.BackgroundColor3=T.Card; runLbl.BorderSizePixel=0
	runLbl.Text="RUN"; runLbl.TextSize=10; runLbl.Font=Enum.Font.GothamBold; runLbl.TextColor3=T.White
	runLbl.TextXAlignment=Enum.TextXAlignment.Center; runLbl.ZIndex=3; runLbl.Parent=card; Corner(runLbl,6)
...
local SLIDE_OUT = TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out)
...
do
	...
	local function buildCard(rec, index)
		...
		local nl3=Label(infoFrame,rec.name,isMobile and 10 or 12,T.White,Enum.Font.GothamBold)
		nl3.Size=UDim2.new(1,-64,0,14); nl3.Position=UDim2.new(0,0,0,7); nl3.TextTruncate=Enum.TextTruncate.AtEnd; nl3.ZIndex=3
		...
		mutLbl2.Size=UDim2.new(1,-64,0,12); mutLbl2.Position=UDim2.new(0,0,0,23); mutLbl2.TextTruncate=Enum.TextTruncate.AtEnd; mutLbl2.ZIndex=3
		local genLbl3=Label(infoFrame,"⚡ "..rec.genText,isMobile and 8 or 10,Color3.fromRGB(80,200,120),Enum.Font.GothamBold)
		genLbl3.Size=UDim2.new(1,-64,0,12); genLbl3.Position=UDim2.new(0,0,0,37); genLbl3.ZIndex=3
		local rarLbl=Label(infoFrame,rec.rarity or "",isMobile and 7 or 9,T.Dim,Enum.Font.GothamBold)
		rarLbl.Size=UDim2.new(1,-64,0,11); rarLbl.Position=UDim2.new(0,0,0,51); rarLbl.ZIndex=3
		local podLbl=Label(infoFrame,(rec.plotOrder or rec.plotName).." • #"..rec.slot,isMobile and 7 or 9,T.Dim,Enum.Font.GothamMedium)
		podLbl.Size=UDim2.new(1,-64,0,11); podLbl.Position=UDim2.new(0,0,0,63); podLbl.TextTruncate=Enum.TextTruncate.AtEnd; podLbl.ZIndex=3
		local selBtn2=Instance.new("TextButton"); selBtn2.Size=UDim2.new(0,54,0,22)
		selBtn2.Position=UDim2.new(1,-58,0.5,-11); selBtn2.BackgroundColor3=T.Card
		selBtn2.BorderSizePixel=0; selBtn2.Text="SELECT"; selBtn2.TextSize=isMobile and 8 or 9
	...
	do
		...
		CreateToggle(SettingsTab.scroll, "Reset on balloon", "auto resets if someone balloons u", function(v) AutoResetBalloonEnabled=v; if v then startARB() else stopARB() end end)
	end
	...
	do
		...
		CreateToggle(SettingsTab.scroll, "Reset when tiny", "auto resets if they shrink u", function(v)
	...
	do
		...
		CreateToggle(SettingsTab.scroll, "Reset on jail", "auto resets if u get jailed/trapped", function(v)
			if v then startJ() else stopJ() end
		end)
	end

	CreateSection(SettingsTab.scroll, "GRAB")
	CreateToggle(SettingsTab.scroll, "Auto Block", "blocks the plot owner right after u grab their brainrot", function(v) AutoBlockEnabled=v end)

	CreateSection(SettingsTab.scroll, "FLASH")
	CreateToggle(SettingsTab.scroll, "Auto Flash", "auto flashes when u get ballooned then goes to grab", function(v)
		AutoFlashEnabled=v
		if v then
			local _afLast=0; local _afPending=false
			local function tryAF()
				if not AutoFlashEnabled then return end
				if not _G._FH_SelectedBrainrot then pcall(ShowToggleNotification,"Auto Flash: pick a brainrot first",false); return end
	...
	CreateToggle(SettingsTab.scroll, "Ragdoll Bypass", "uses ragdoll on them via admin panel before grabbing", function(v)
		RagdollBypassEnabled=v
	end)

	CreateToggle(SettingsTab.scroll, "Anti Ragdoll", "stops ragdoll from working on u at all", function(v)
		if v then AntiRagdoll.enable() else AntiRagdoll.disable() end
	end)

	CreateSection(SettingsTab.scroll, "PROTECTION")
	CreateToggle(SettingsTab.scroll, "Anti Admin Panel", "blocks tiny jail ragdoll giant and inverse on u", function(v)
		AntiAdminEnabled=v
	end)

	CreateSection(SettingsTab.scroll, "BRAINROTS")
	CreateToggle(SettingsTab.scroll, "Auto Select Best", "auto picks the highest gen brainrot when the list updates", function(v)
	...
	do
					...
					for _,cmd in ipairs(FP_CMDS) do
						...
						for _,e in ipairs(fpCoolBtns[cmd.name]) do
							...
							if btn2 and btn2.Parent then
								if onCD and cdTxt then btn2.Text=cdTxt; btn2.TextSize=10; btn2.TextColor3=Color3.fromRGB(255,80,80); btn2.BackgroundTransparency=0.65
								else btn2.Text=emoji; btn2.TextSize=isMobile and 14 or 17; btn2.TextColor3=T.White; btn2.BackgroundTransparency=0.3 end
							end
						end
					end
					task.wait(0.25)
		...
		local function fpBuild()
			...
			Corner(win,10)
			local ws=Instance.new("UIStroke"); ws.Thickness=1.5; ws.ApplyStrokeMode=Enum.ApplyStrokeMode.Border
			...
			local tl2=Label(hdr2,"⚡ Quick Panel",isMobile and 11 or 14,T.White,Enum.Font.GothamBold)
		...
		local function fpMakeRow(plr, order)
			...
			local nl4=Label(row,dText,isMobile and 10 or 12,T.White,Enum.Font.GothamBold)
		...
		CreateToggle(SettingsTab.scroll, "Quick Panel", "opens a panel to use admin commands on anyone in the server", function(v) fpShow(v) end)
	end
	...
	CreateButton(SettingsTab.scroll, "Save Config", "saves all your toggle states", function()
...
local _uiAnim = false
local SHOW_TI = TweenInfo.new(0.28, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
local HIDE_TI = TweenInfo.new(0.22, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
...
pcall(ShowToggleNotification, "Vertex Pvp loaded", true)
