...
local WIN_W = isMobile and 260 or 300
...
local HDR_H = isMobile and 44 or 42
...
DiscordBox.Size=UDim2.new(0,isMobile and 80 or 120,0,isMobile and 14 or 16)
...
TitleLbl.Size=UDim2.new(0,140,0,20); TitleLbl.Position=UDim2.new(0,8,0,isMobile and 16 or 18); TitleLbl.ZIndex=6

local VerLbl = Label(Hdr, "v2.0 | r9qbx", isMobile and 7 or 9, T.Dim, Enum.Font.Gotham)
VerLbl.Size=UDim2.new(0,100,0,12); VerLbl.Position=UDim2.new(0,8,1,-14); VerLbl.ZIndex=6
...
do
	...
	local foldBtn, foldStroke = styleBtn("▼", 1)
	foldBtn.MouseButton1Click:Connect(function()
		...
		if _guiFolded then
			...
			if TBLine then TBLine.Visible = false end
			Win.Size = UDim2.new(0, WIN_W, 0, HDR_H)
		else
			foldBtn.Text = "▼"
			if TabBar then TabBar.Visible = true end
			if ContentArea then ContentArea.Visible = true end
			if TBLine then TBLine.Visible = true end
			Win.Size = UDim2.new(0, WIN_W, 0, WIN_H)
		end
	end)

	local closeBtn, closeStroke = styleBtn("✕", 2)
	...
	closeBtn.MouseButton1Click:Connect(function()
		if not _minimized then
			...
			_miniFrame.Size = UDim2.new(0,80,0,28)
			_miniFrame.Position = UDim2.new(0,10,0.5,-14)
			...
			Corner(_miniFrame, 8)
			Stroke(_miniFrame, T.AccentBright, 1.5)
			local _mts
	...
	_G._FH_ResetBtnEntry = makeActionBtn("RESET", 5, function()
		local lp = Players.LocalPlayer
...
TabBar.Size=UDim2.new(1,0,0,34); TabBar.Position=UDim2.new(0,0,0,HDR_H)
...
TBLine.Size=UDim2.new(1,0,0,1); TBLine.Position=UDim2.new(0,0,0,HDR_H+33)
...
ContentArea.Size=UDim2.new(1,0,1,-(HDR_H+35)); ContentArea.Position=UDim2.new(0,0,0,HDR_H+35)
...
local function CreateTab(name)
	...
	local sc=Instance.new("UITextSizeConstraint"); sc.MaxTextSize=11; sc.MinTextSize=5; sc.Parent=lbl
