...
local lockBtn,lockStroke=makeHdrIconBtn("ðŸ”’",1)
...
local foldBtn,foldStroke=makeHdrIconBtn("â–¼",2)
local _guiFolded=false

local closeBtn,closeStroke=makeHdrIconBtn("âœ•",3)
...
local function mkESP(plr)
	...
	task.spawn(function()
		...
		if hasAP then
			...
			apL.TextColor3=T.Green; apL.Text="AP âœ“"
		else
			apF.BackgroundColor3=Color3.fromRGB(22,6,6)
			Stroke(apF,T.Red,0.8)
			apL.TextColor3=T.Red; apL.Text="AP âœ—"
		end
	end)
end
...
do
	...
	local function buildCard(rec,index)
		...
		local genLbl3=Label(infoFrame,"âš¡ "..rec.genText,isMobile and 8 or 9,T.Green,Enum.Font.GothamBold)
		...
		local podLbl=Label(infoFrame,(rec.plotOrder or rec.plotName).." â€¢ #"..rec.slot,isMobile and 7 or 8,T.Dim,Enum.Font.GothamMedium)
		...
		labelUpdaters[uid]=function(nr,ni)
			...
			genLbl3.Text="âš¡ "..nr.genText; rarLbl.Text=nr.rarity or ""
			podLbl.Text=(nr.plotOrder or nr.plotName).." â€¢ #"..nr.slot; card.LayoutOrder=ni
		end
	end
	...
	do
		...
		local FP_GAP=3
		local FP_HDR_H=24
		local FP_CMDS={{name="tiny",emoji="ðŸœ"},{name="jail",emoji="ðŸ”“"},{name="rocket",emoji="ðŸš€"},{name="ragdoll",emoji="ðŸƒ"},{name="balloon",emoji="ðŸŽˆ"}}
		...
		local function fpBuild()
			...
			mb2.Text="â€”"; mb2.TextSize=8; mb2.Font=Enum.Font.GothamBold
		...
		local function fpMakeRow(plr,order)
			...
			task.spawn(function()
				...
				if hasAP then
					apRowBadge.Text="AP âœ“"; apRowBadge.TextColor3=T.Green
					TweenService:Create(apRowBadge,F,{BackgroundColor3=Color3.fromRGB(6,20,10)}):Play()
				else
					apRowBadge.Text="AP âœ—"; apRowBadge.TextColor3=T.Red
